...
test('⭐⭐ the tempo mark’s hit-box is a TEXT LINE, and does not swallow the trill under it', async ({ score }) => {
  const { tempo, trill } = await score.evaluate(async () => {
    ...
    h.engine.trill.addTrill({ startNoteId: id })
    h.engine.tempo.addTempoMark(1, { beat: h.frac(0, 1), text: 'Allegretto ♩ = 60', unit: 'q', bpm: 60 })
    await h.render()
...
async function belowStaffPair(score: import('@playwright/test').Page) {
  return score.evaluate(async () => {
    ...
    h.engine.dynamic.addDynamic(1, { beat: h.frac(3, 1), text: 'p' })
    await h.render()
    const stave = h.staves()[0]
...
test('⭐⭐ a POSITIVE tempo offset lifts the mark AWAY from the staff', async ({ score }) => {
  const { before, after, lower } = await score.evaluate(async () => {
    ...
    const mark = h.engine.tempo.addTempoMark(onFirstRow + 1, { beat: h.frac(0, 1), text: 'Allegro' })!
    await h.render()
    const y = () => h.placed('g.tempo text')[0].y

    const before = y()
    h.engine.tempo.nudgeTempoOffset(mark.id, 0, 2)   // +2 staff-spaces OUTWARD
    await h.render()
    const after = y()
    h.engine.tempo.nudgeTempoOffset(mark.id, 0, -4)  // …and back down past where it started
    await h.render()
