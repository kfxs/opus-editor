...
test('🚨🚨 a DYNAMIC under a covered note does not touch the arch — his report, 2026-08-21', async ({ score }) => {
  ...
  const r = await score.evaluate(async () => {
    ...
    return {
      ...
      dynamics: h.placed('g.annotation text').length,
    }
  })
...
test('⭐⭐ a continuation starts AFTER the clef, key and meter (Gould p. 112, verbatim)', async ({ score }) => {
  ...
  const r = await score.evaluate(async () => {
    ...
    const clefs = h.inkSizes('g.clef').filter(b => onLine(b.y + b.height / 2))
    const sp = (h.staves()[0].bottom - h.staves()[0].top) / 4
...
test('⭐⭐ the WHOLE curve moves RIGIDLY — its shape is not re-solved (his ask, 2026-08-18)', async ({ score }) => {
  ...
  const r = await score.evaluate(async () => {
    ...
    const sp = (h.staves()[0].bottom - h.staves()[0].top) / 4
    const sample = () => h.curveSamples('g.slur path', 60)
    const before = sample()
    ...
    const arcTop = Math.min(...h.curveSamples('g.slur path', 20).map(p => p.y))
