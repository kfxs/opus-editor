...
>
> 📄 Read with `docs/font-metrics-plan.md` (P2 — how the ink table is generated from a font) and
> `docs/own-engraving-engine.md` (why the drawing is moving to us at all). ⚠️ Everything below was
...
**outlines** the glyphs (`docs/pdf-export.md`), i.e. the font's shapes end up inside a document the
...
editor will have: **ask the font; fall back to our own rule when it is silent.** See `docs/clef.md`
