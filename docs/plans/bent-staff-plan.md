...
| 21 | ⭐ **GRACE NOTES** — acciaccatura, appoggiatura (and their slash, beam, size) | BLOCK (with or before its host) | ⛔ seen missing 2026-09-26 (his ask: *"what about grace notes, parenthesis, cue and the new things?"*) — the page draws them in their own pass (`GracePass`), not through `NoteBuilder`, so nothing reaches the spine | the host note + its graces as one block (the tuplet's lesson), `GracePass`'s own drawing inside; check the spacing already buys their room |
| 22 | **BRACKETED grace** | BLOCK | ⛔ seen missing 2026-09-26 (⏸️ the feature itself is PARKED by his call) | after #21, the same way |
| 23 | **PARENTHESISED note** — its brackets | BLOCK (part of the note's) | ◐ seen 2026-09-26: the note draws, its BRACKETS do not | find where the page draws them (a pass after the note, like the graces) and draw it inside the note's block |
...
12 staves → B (17–19). ✅ 5 · 6 · 7 · 8 · 13 · 14 done by 2026-09-25, 11 · 16 on 2026-09-26 — ⭐ #13 set the pattern #15 and #16 reuse: solve in `(s, d)`,
