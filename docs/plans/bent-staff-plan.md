...
| 29 | ⭐ **FANNED (feathered) BEAMS** — the fan, its members' pitches, cue fans, a fan joined to a group, a fan across a barline | BLOCK spanning the slot's territory | ⛔ (his ask, 2026-09-26: *"do we have the fan in the spine plan? if not we have to include it"* — it was only three words of row 10). Today a slot holding a fan draws as a LONE block with no fan (`spineScore`: *"a group holding a FAN gets no Beam there … its notes stay lone blocks"*), and its MEMBERS' pitches — drawn by the page outside the slot's note — do not draw at all | the page's own `beams/FanPass.drawFannedBeams`, the grace pass's way (#21): called inside a block, narrowed to the fields it uses — ⚠️ but it also reads `pass.solvedColumns` (the bar's column x's, where the members spread from the head to the NEXT note) and `staveNoteMap`, so the block is the WHOLE fan (host to next column), its members laid out along the path from the spine's own columns (`eye/spineSpacing`) — option (b) of §6: heads on the path, the ramp lines straight. Then: a fan JOINED to the group on its left (one block with that group), a fan across a barline (`drawCrossBarFanBeams`, with #10's cross-bar beams), and slurs/ties to a MEMBER through the recorded anchor (`fanMemberAnchorMap`, the graces' route) |
