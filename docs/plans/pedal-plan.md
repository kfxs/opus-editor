...
- **No `type`.** Sustain is the only pedal we build. When sostenuto and una corda arrive they add
  ...
  (no migration, ever — `docs/plans/json-io-plan.md`), and no JSON written today becomes wrong.
...
(`docs/plans/ottava-plan.md` §1) — is about octave lines, and the only report of her saying the same for
...
off voice colours together. See `utils/selectionColors` for the rule and `docs/plans/ottava-plan.md` for
the same note.
...
- **`HighlightController.applyPedalSelectionHighlight()`** — what the `ELEMENT_SPECS` row's
  `highlight` calls. ⭐ PAINT, don't RECOLOUR (`docs/how-it-works/barline-selection.md`).
...
signs, and *a press may only reach INK* (`docs/how-it-works/barline-selection.md`). So: the two glyph
...
kind-agnostic guide (`docs/plans/dynamic-offset-plan.md`). Two edits: `drawPedal` registers it on the FIRST
...
The ottava's bill, verbatim (`docs/plans/ottava-plan.md` §4). ⚠️ An earlier draft counted four; reading
...
- ⭐⭐ **TWO KINDS OF VERTICAL**: inside its own staff's room the `y` is plain INK (bounded by the page
  ...
  the halfway line and never at the pentagram (his rule, 2026-08-19). ⚠️ **UNCHANGED, and deliberately**: the
  ottava's drag grew a white-space GATE on 2026-08-30, and the pedal is not gated — his report the
  moment it was (*"the pedal is completly crazy"*, eight hand-overs down the page) is why the gate is
  opt-in, in `docs/plans/mark-drag-and-reanchor.md`. The lift comes back OUT before
...
they rule out, and the three next steps are in `docs/history/render-performance-plan.md` §12.6 — ⛔ nothing
