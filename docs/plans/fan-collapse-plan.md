...
docs/plans/fanned-beams-plan.md §0 opens *"you enter the time first"*: you type one ordinary note that
...
- ⭐ **The ROOM** — his first report on the finished feature, and the fix is not in this file:
  `engine/rendering/fanRoom.ts` (see the entry in docs/plans/fanned-beams-plan.md §4). The collapse is
  ...
  docs/plans/fanned-beams-plan.md §5.
...
- **Un-collapsing** — "give me my seven notes back". Removing the mark leaves the one carrier note,
  ...
  (docs/plans/fanned-beam-pitches-plan.md, the same answer given to dissolving a fan into N slots).
