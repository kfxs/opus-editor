...
percussion (`docs/plans/unpitched-staves-plan.md`), these are ordinary clefs: their staff lines
...
- Note placement is untouched. VexFlow's `'8vb'` annotation is cosmetic — it swaps the clef
  ...
  `rendering/engraved/EngravedClef` + `engrave/header/clefSign`; the point stands: the octave clef is a
...
3. The six `addClef` sites pass VexFlow's annotation — `stave.addClef('treble', size, '8vb')`.
   ...
   annotation any more: a clef is our `rendering/engraved/EngravedClef` (added through `addClefSign`), whose
...
transposition lands (`docs/plans/instruments-plan.md`), this stops being about octaves — a Bb
...
`score.clef` happened (`docs/plans/clef-model-plan.md`).
