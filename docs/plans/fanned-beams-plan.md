...
> each gap is the spacing rule applied to that member. Read `docs/plans/spacing-model-plan.md` §P5 for what
...
(`FanMemberChord.articulations`, docs/plans/fanned-beam-pitches-plan.md §3) and the engraving had followed
...
group" (docs/plans/fan-collapse-plan.md), but a selection *containing a fan* cannot be collapsed — a fan
...
- **Per-note pitch** — ⏭️ now planned in **docs/plans/fanned-beam-pitches-plan.md**. ⚠️ And the guess made
...
- **Making a fan out of notes you already typed** — ✅ now BUILT, in **docs/plans/fan-collapse-plan.md**.
...
- **Per-member horizontal spacing** — ✅ now BUILT, in **docs/plans/note-spacing-plan.md §7**. Spacing a
...
- **Per-member horizontal OFFSET** — P0–P1 DONE, in **docs/plans/note-offset-plan.md** ("Inside a FAN").
...
- **How far the wedge OPENS** — ✅ now BUILT, in **docs/research/fan-beam-spread.md**. The lines step apart by
...
a whole-editor priority: **`docs/plans/spacing-model-plan.md`** (evidence in
`docs/research/spacing-model-research.md`). ⭐ It ends this specific symptom by construction: a fan's members
