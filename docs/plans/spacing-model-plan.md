...
> side, and the six measurements taken in our own code — is **`docs/research/spacing-model-research.md`**.
...
> loose for a piece of crotchets. Decided, not yet built: `docs/plans/shortest-duration-plan.md`, whose §9
...
the measured numbers off our own drawing, in `docs/plans/staff-size-plan.md` §6a — read it before changing
...
(`docs/plans/note-spacing-plan.md` §4.3) — the model makes it structural instead.
...
documented bug class (*visual coords inside a scaled scope*, `docs/plans/staff-size-plan.md`), and
...
`docs/research/spacing-model-research.md` §6f. Two details there that this section does not have: their spring
...
- 📄 **The numbers are `docs/research/spacing-model-research.md` §6.** The 12 px inset needed no subtracting
...
- **Empty bars get WIDER, into an open complaint.** `EMPTY_LANE_NOTE_SPACE` is 40 px;
  ...
  treating its stretch differently), or `docs/plans/bar-width-plan.md` "Known issues" #1 is answered first.
...
- ⭐ **An EMPTY bar keeps `EMPTY_LANE_NOTE_SPACE`, and the rule does not govern it** — the open
  ...
  not pushed on. `docs/research/spacing-model-research.md` §6c has the measurements, the sources, and the
...
(`docs/history/vexflow-boundary.md` §6).
...
`docs/history/vexflow-boundary.md` §6, and `e2e/spacing.e2e.ts`'s *"the SAME DURATION is drawn the same
...
- ⚠️ The **preview ghost** formats its own temporary stave and must run the same pass, or a ghost sits
  where the note will not (already a known gap — `docs/plans/note-spacing-plan.md` §4).
...
docs/research/spacing-model-research.md §6e.
...
by argument** — `docs/plans/beam-engraving-plan.md` P4b was the first. Both remain open on purpose:
...
| …or the OTHER even | ⭐ **his own axis**, `docs/plans/shortest-duration-plan.md` §9.5: *"the second group can have more space so it looks more even matching with the first group… it will look more beautiful"* — LilyPond's `proportionalNotationDuration`, pure elapsed time | ✅ row `proportional` — **ratio 2**, and it reproduces that plan's measured 1.80 for his bar's quavers |
...
forms where two staves are not meant to line up at all. `docs/research/20c-notation-survey.md` §4/§5 is the
...
| | date |
|---|---|
| `docs/research/spacing-model-research.md` written | **2026-07-30** |
...
✅ **AND THE BOOK HALF LANDED THE SAME DAY.** Written up in `docs/research/spacing-model-research.md`
...
🚨 `docs/plans/beam-engraving-plan.md` records the identical trap in the **shape** key, found the same day
...
- ✅ **Does a FAN space linearly? — NO. Decided 2026-07-30 (his call).** The question was whether a
  ...
  than a second rule for one gesture. ⚠️ It also leaves `docs/plans/note-spacing-plan.md` §7.3's worry —
