...
It is the sibling of the accidental stamp (`docs/plans/accidental-stamp-plan.md`) — read that first;
...
x from the notehead. (docs/plans/two-note-tremolo-plan.md §2 reached the same split from the other side.)
...
- **Two-note (between-notes) tremolo.** A different notation with a different model — it belongs
  ...
  → **Planned: `docs/plans/two-note-tremolo-plan.md`.**
...
- **Keypad wiring.** The page-2 tremolo keys are still `momentary` (`docs/how-it-works/keypad.md`). They arm
