...
- ⭐⭐ **WRITTEN pitch.** The field is split on it — Dorico, LilyPond and MusicXML store the sounding
  ...
  written**, matching the answer `docs/plans/octave-clefs-plan.md` already gave. §2.
- ⭐⭐ **The LADDER GOES FIRST.** `docs/how-it-works/above-staff-ladder.md` §4 named the 8va bracket as one of the
...
2. ⭐⭐ **The line ends AT THE LAST NOTE, not at the end of its duration.** Gould: continuing an
   ...
   `docs/plans/trill-plan.md` §4). An ottava that reused it would be wrong by the book.
...
6. ⭐ **They continue across system and page breaks, and the numeral is shown again at the start of
   ...
   which turned out to be G. Schirmer's manual and nobody else's (`docs/plans/trill-plan.md` rule 6). His
...
`docs/plans/octave-clefs-plan.md` §1: *"the model keeps storing WRITTEN pitch — what the notehead says. The
...
`'8vb'` annotation (that one belongs to `docs/plans/octave-clefs-plan.md`, not here).
...
had to pay (`docs/plans/trill-plan.md` §2.1).
...
`docs/how-it-works/above-staff-ladder.md` §4 wrote the trigger down in advance: *"the first family that is
...
first consumer. ⭐ This does not contradict `docs/how-it-works/above-staff-ladder.md` §3's "the trill is the
...
- ⛔ **Not a priority-NUMBER table.** `docs/how-it-works/above-staff-ladder.md` §2 is still right: while the inner
...
- ⭐ **The dynamics line (and the trill) become PRODUCERS of the occupied band** — not rows in
  ...
  piece of P0, and it is a near relative of the fix `docs/how-it-works/above-staff-ladder.md`'s "Known and
...
(`docs/plans/trill-plan.md` §7), and applied where the midi is derived, not where each caller feels like it.
...
⭐⭐ **And it is the same seam `docs/plans/octave-clefs-plan.md` §2.4 needs.** That plan's one genuinely new
...
5. ✅ **The continuation label** — he has already asked for parenthesised, and §1 rule 6 says that is
   ...
   what a bracketed reminder IS — and `docs/plans/trill-plan.md` rule 6 literally cites "where an `(8)`
...
- ✅ **P5 — ENTRY. BUILT 2026-08-13.** `MusicEngine.createOttava` + `ottavaOps.addOttavaOverNotes`,
  ...
  - ⭐ **THE ATTACHMENT GUIDE** — a selected bracket draws the dashed line to where it is anchored
    (the fifth kind on the kind-agnostic guide, `docs/plans/dynamic-offset-plan.md`). Two edits:
...
Four asks in one afternoon, and the shape is the HAIRPIN's (`docs/plans/dynamics-line-and-hairpins-plan.md`,
...
⚠️ The outer limit — off the **page** — is not this file's: see `docs/plans/engraving-overrides-plan.md` §8, the
...
- ⭐ The typed value goes through **`nudgeOttavaEndpoint`** (absolute → `next − current`), never
  ...
  (`docs/plans/engraving-overrides-plan.md` §8). ⛔ A controller writing the override itself would be a
...
being re-decided by eye (2026-08-30) — see `docs/plans/mark-drag-and-reanchor.md`: a DRAG now WAITS for the
...
measurements and the four cuts that failed are in `docs/plans/mark-drag-and-reanchor.md`. ⛔ The keyboard is
untouched.
...
- ⚠️ It USED to arrive where the engraver would put it, both axes of the offset dropped. ⛔ No longer
  (2026-08-30, exploratory): *"a reanchor should not jump, should keep the same mark position as
  offset"* — the offset absorbs the anchor's travel instead, so the drawing stays under the hand.
...
⭐ The fix and its reasoning live in `docs/plans/slur-endpoint-offset-plan.md` §"The rule
...
drop renders for real. See docs/history/render-performance-plan.md §12.5a for the measurement and for why it
read as the latch.
...
docs/plans/slur-endpoint-offset-plan.md §"The mouse" carries the sources and the reasoning, and ⛔ the whole
