...
- `noteOn(midi, when, durationSec, velocity)` — schedule one sounding note.
  📄 ⏭️ **This signature is the INTERPRET step in disguise** (`docs/plans/playback-semantics-plan.md`,
...
- **Phase 2 — GM instrument switching (later).** ⚠️ **SUPERSEDED by
  `docs/plans/instruments-plan.md` — do NOT build the per-staff field sketched below.**
