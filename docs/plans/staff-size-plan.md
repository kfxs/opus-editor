...
he reported is the duration law and belongs to `docs/plans/shortest-duration-plan.md`.
...
reference duration (`docs/plans/shortest-duration-plan.md` — a real defect at the dense end, and not this).
...
⭐ This is the trap `docs/history/render-performance-plan.md` §7a names in the abstract — *reuse the wrong
...
> `interactions/controllers/staffSizeToggle.ts` (target / light / press — it left `dev/` when the demo's Staff
