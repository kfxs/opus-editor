...
`Shift`+a letter. Continues docs/plans/fanned-beams-plan.md, which deferred exactly this.
...
- Cross-barline fans, MusicXML, and the standing list in docs/plans/fanned-beams-plan.md §4.
...
- **A member's sign clears the member's own ledger lines**, by the rule in
  `docs/research/accidental-ledger-clearance.md` — his second report, from the same page, and general rather
...
Unchanged from docs/plans/fanned-beams-plan.md §1: every number here is provisional and expected to be
