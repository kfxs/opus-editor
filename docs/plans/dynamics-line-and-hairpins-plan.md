...
**Beside this:** `docs/plans/dynamic-voice-scope-plan.md` — which voices a wedge governs (absent = ALL of
...
(`docs/plans/json-io-plan.md`). Worth a line only so a hand-nudged mark sitting somewhere new in the
...
`docs/plans/dynamics-plan.md` §8 already reserved the shape: *"a separate `Hairpin` array on `Measure`,
...
where it is anchored — the fourth kind on the kind-agnostic guide (`docs/plans/dynamic-offset-plan.md`),
...
📄 **And what the ramp should be a ramp OF is now written down: `docs/plans/playback-semantics-plan.md`**
...
- **The playback ramp** (§9) — needs `resolveChordLevels` to return a number rather than an enum;
  ...
  and a violin cannot share one realisation: `docs/plans/playback-semantics-plan.md`.
...
docs/plans/font-metrics-plan.md under P5 — draw staves at the font's 0.13 and the hairpin's 0.13 IS the
...
during the render** (docs/history/render-performance-plan.md §12.5a), so it answers *"whose staff?"* as of the
