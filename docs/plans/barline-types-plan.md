...
- **The selection names a boundary** — `{ kind: 'barline', measure: N }`, *the line that ENDS bar N*
  (`interactions/elements/barline.ts`, `docs/how-it-works/barline-selection.md` §1).
...
`docs/how-it-works/barline-selection.md`'s "Out of scope" names repeats/double/final bars with the reason to keep
...
- **Optional, absent = a plain line** — `docs/plans/json-io-plan.md`: *"a new feature lands as an optional
...
⚠️ Contrast `docs/research/clef-research.md` §4.3, where four books put a cautionary **clef** *before* the
...
`docs/history/vexflow-boundary.md`: *take a decision from VexFlow when there is a **RULE we want to state and
...
And `docs/plans/own-engraving-engine.md` states the rule directly, as the one to add beside the boundary
test:
...
- ⭐ **a `validateBarlines` at the load boundary.** `ScoreModel.fromJSON` is a bare `JSON.parse`
  ...
  disagree (`docs/plans/json-io-plan.md`);
...
(docs/how-it-works/barline-selection.md, *"the coordinates LIE"*). Pinned by an e2e that nudges staff spacing and
...
asymmetry they were aimed at is still there and is **`docs/plans/bar-width-plan.md` "Known issues" #1's
...
RECOLOUR* is `docs/how-it-works/barline-selection.md` §3a.
...
ENTIRELY on 2026-08-28: `docs/plans/barline-join-plan.md`, with `docs/research/barline-join-research.md` under it.**
...
are `docs/research/20c-notation-survey.md` §4 and our own model.
...
| **no music** | a passage that engraves nothing | `docs/plans/rest-hide-plan.md` — a hidden rest *"is not removed — it is still real musical content (an empty beat must stay filled), it just stops engraving as a normal black glyph"* |
...
only reach ink** (`docs/how-it-works/barline-selection.md` §1a — a culled bar's invisible hit-box answering a
