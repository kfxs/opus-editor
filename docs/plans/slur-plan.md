...
- **Ties are the structural anti-pattern to avoid copying, but the right *rendering/selection*
  ...
  - **Highlight (do NOT reuse `colorTieArc` verbatim):** `HighlightController.applyTieHighlight` +
    ...
    element's own SVG group (see docs/how-it-works/note-selection-hit-detection.md). `applySlurHighlight` must
...
> `docs/plans/tuplet-control-plan.md` §4b. The "sets an explicit placement / toggles the explicit side"
...
session — see `docs/plans/multi-voice-plan.md`):
...
> pp. 35–39** — now read in `docs/research/slur-tie-research.md`. ⭐ **A "not obtainable" line is a fact about
...
coordinates are not one ruler (the hairpin taught us this the hard way, `docs/plans/dynamics-line-and-hairpins-plan.md`).
...
(`docs/plans/dynamics-line-and-hairpins-plan.md`, *a late-start wedge's `endX < startX`*). The rise comes
...
`docs/plans/element-copy-paste-plan.md`, which owns the rule (a PASSAGE keeps a mark's overrides, a lone
