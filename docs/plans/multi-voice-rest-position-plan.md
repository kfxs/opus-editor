...
>
> 📄 **The evidence is in `docs/research/multi-voice-rest-position.md`** — Gould/Ross/Gerou & Lusk/Stone with
...
+  // Multi-voice rest line, derived (docs/plans/multi-voice-rest-position-plan.md). Single voice: 0.
```
...
`docs/plans/own-engraving-engine.md` §3.1 — with the LAYOUT-vs-ENGRAVE question it raised recorded as an
...
- `docs/plans/multi-voice-plan.md` §13 (`:284-292`) states the four-lane ladder as *the* rule.
...
- 🚨 **Three and four voices are UNKNOWN in every source** (research §8), and `dir` is voice PARITY,
  ...
  conventions from down to top v4-v2-v1-v3 right?"* He is right, and `docs/plans/multi-voice-plan.md` §13
