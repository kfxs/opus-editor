...
**Sibling:** `docs/plans/element-copy-paste-plan.md` — Ctrl+C/Ctrl+V of ONE selected element (an
...
- **Partial overlap → fully-enclosed only.** A slur travels only if BOTH endpoints
  ...
  SELECTED to travel (`MarkFilter`, `docs/how-it-works/passage-selection-marks.md`). A box selects everything it
...
- [x] **Phase 2 — dynamics travel. DONE.** `buildClipboardFromSelection` captures
      ...
      `ClipDynamicInput` copy is gone — since Phase 4 of docs/history/refactor-plan-2026-07-27.md the one
