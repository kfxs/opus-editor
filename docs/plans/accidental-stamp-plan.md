...
(`docs/plans/articulation-stamp-plan.md`) — read that first; this doc records only where accidentals
differ.
...
- **Selected note highlights its articulations.** `highlightNote` now also colours the note's
  ...
  its own — `docs/plans/note-engraving-plan.md` §1g.6. ⭐ Nothing here changes because every selector in
