...
⚠️ **At most ONE tempo mark per beat** (`docs/plans/tempo-marks-plan.md` §4). A paste **replaces** the
...
**Sibling:** `docs/how-it-works/passage-selection-marks.md` — what a passage box selects (and highlights), and
...
upsert — the CLEF's rule, not the wedge's (docs/plans/ottava-plan.md §7.8): one (beat, staff) may hold at
...
gesture, *lift, re-press* (docs/plans/pedal-plan.md §3.3): a pedal still down when this one begins is
...
- ~~🚨 **A PASSAGE copy still loses a wedge's shape.**~~ ✅ Done 2026-08-20, for every mark kind — see
  ...
  orphans unless it rides the capture/restore seam (docs/plans/dynamic-offset-plan.md, P1).
