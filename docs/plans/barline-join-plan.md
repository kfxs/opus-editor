...
`docs/research/barline-join-research.md` (three agents, 2026-08-28, sourced) — ⛔ read it before re-asking any
...
- **Absent = the default** (§2.3), so a fresh score stores nothing and old JSON keeps loading:
  `docs/plans/json-io-plan.md`'s *"a new feature lands as an optional field whose absence is a legal score"*.
...
INSTRUMENT question and we have no instrument object — `docs/plans/instruments-plan.md` is where it lands.
...
- ⛔ **We do NOT add a positional field now.** A field with no feature is a field nothing maintains —
  ...
  `docs/plans/barline-types-plan.md` did the opposite (it stores a staff scope nothing reads) and §10 lists
...
- 🚨 **`invisible` AND THE AUDIENCE.** `drawSign` ends with `applyHiddenTreatment(group, audience)`.
  ...
  and PRINTS — the one thing `docs/how-it-works/pdf-export.md`'s *hidden = gray on screen, OMITTED in print*
...
- **JSON:** one optional field, absence legal, ⛔ no migration — `docs/plans/json-io-plan.md`.
...
Full record + citations: `docs/research/barline-join-research.md`. The five findings this plan rests on:
