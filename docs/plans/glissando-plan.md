...
| **G8** | **text OFF by default** | ✅ 2026-09-25 (his brief) | no word drawn until Properties turns it on; `text?: string` (absent = none) — ✅ built (the **Text** phase) | ⚠️ every app defaults to AUTO (industry §7.3); Gould wants the word at least the first time; Stone says the line suffices. His call stands |
...
| `interactions/stamps/glissandoTool.ts` | `pressGlissando` / `glissandoLit`: selected heads ⇒ make one per head; nothing selected ⇒ ARMS the stamp `{ kind: 'glissandoLine' }` — ⭐ the blue cursor, ⛔ no ghost line (his call); a click on a head adds one (`glissandoStamp`, additive only, via the `noteMarkStamps` table); pressed again ⇒ disarm. `devToolbar` gets ONE `toggle(…)` row, as `enclosureTool` is wired (`devToolbar.ts:252`) |
...
| **P4** ✅ built | selection + Delete + highlight: `interactions/elements/glissando` (hit on the drawn STROKE, ⛔ not its box — a steep line's box is mostly air; 8 px along the line, 4 px near its ends where a head stands — his *"more user friendly"*), a row in `ELEMENT_SPECS` + `ELEMENT_HIT_ORDER` (between the tie and the slur: the thinner target), each piece registered as `'glissando'` by `GlissandoRenderer`. ⚠️ The kind is `glissandoLine`, ⛔ not `glissando`: `lint:hubs` reads kind names as words and `MusicEngine` already says `glissando` (the `headEnclosure` precedent). Delete removes the line and selects its note; the Properties report carries the object + its derived target; an element paste anchors on its note | click the line, Delete |
| **Properties** ✅ built | `windows/properties/panels/glissando` → `bus.glissandoEdit` → `GlissandoEditController` → `engine.glissando.setSide / setEnd / setDirection` (one undo entry each): **side** (after / before), **goes to** (the next note / nothing — ⛔ not for a line into the note), **direction** (up / down — ⭐ only when an end is free, his ask). The dropdown moved to `rows.buildSelect` (the tuplet's, now shared). ⭐ Also (his asks, same day): a SELECTED NOTE lights its glissando (`elements/glissando.paintNoteGlissando`, called by `notePaint` beside the tie's), and the `gliss` button TOGGLES — any selected head without one ⇒ all get one, all with one ⇒ all lose it (`engine.glissando.toggle`, the brackets' rule, one undo step) | select a glissando, change it in Properties |
| **Text** ✅ built | `Glissando.text` (absent = none — his brief), Properties free text with presets (gliss. · port. · glissando), one undo step, carried by paste. Drawn ITALIC along the line (`textRoles.glissandoText`, words face, 1.61 sp = MuseScore's size — no book states one), centred, 0.35 sp above it (`GLISSANDO_TEXT_RULES.gould`, p. 141 (a); `musescore` 0.1), and ⭐ ONLY where the line is long enough for the word + 0.25 sp each side (his rule: *"only if we have enough space"*; the margin is ours) — a broken line puts it on the first piece that fits. `__gliss.textPlace(…)` | select a glissando, type `gliss.` in Properties |
| **Stamp** ✅ built | ⭐ his ask: in Select mode with NOTHING selected, `gliss` arms `{ kind: 'glissandoLine' }` — the BLUE CURSOR (`scoreCursorClass`), ⛔ no ghost line (his correction: *"no ghost line i said the blue cursor"*). A click on a head adds one (`stamps/glissandoStamp`, ⛔ additive only — a head with one is left alone); a rest or paper is consumed, no change; `gliss` again disarms. The four note-mark stamps MouseController called one by one are now ONE table, `stamps/noteMarkStamps` (articulation · dot · tremolo · brackets · glissando) | nothing selected, `gliss`, click heads |
| **later** | ⛔ a decision list, ⛔ not a queue: Properties still owed (style wavy · text level instead of along — Gould allows both) · end-handle drag + offsets (the slur's `SlurEndpointOffsetOverride` shape) · playback (G14) · pinned target / other staff (G6) · arrow cap (Gould p. 143/340) · curved contour (Gould p. 146, 358) · jazz (G15) · the bracketed finishing pitch (G16) · text repeated on a continuation piece (Dorico repeats it; the trill's `continuationLabel` shape) · MusicXML mapping (engines §6.8) | — |
...
⚠️ **Found 2026-09-25 (his report, a C4 → G♯4 line):** the Gould row stopped short of the ♯ but ended at the
HEAD's height — her two options mixed. ✅ Fixed as a row field, `GlissandoEndRule.accidental`: `truncate` (hers,
the line still aimed at the head and cut short — Verovio's too) vs `reangle` (MuseScore, LilyPond). Then, on his
*"isn't the line too short?"*: her plate MEASURED (p. 141, 600 dpi) — the stop is **0.79 sp**, not the 0.7 read by
eye; so the gap is hers, and the short line is the tight eighths + MuseScore's 1.2 sp minimum (her plate's line
is ≈3.9 sp). ⏳ His call whether the minimum or the gap moves.
✅ **His call the same day: a `house` row, ARMED** — *"for the default I like gould angle but the empty space
should be shorter"*: Gould's row with the stop before an accidental at 0.3 sp (her head gap), ⚠️ a first guess;
`__gliss.accidentalGap(n)` is the knob for his eye. And on *"double check all angles"*, every engine re-read from
SOURCE (`glissando-engines-research.md` §5b): MuseScore and Verovio AIM THROUGH THE HEAD CENTRES and cut back along
the line (MuseScore truncates — our first row wrongly re-angled; Verovio slides forward under the sign); LilyPond
runs edge to edge and re-angles to the accidental. New row fields `aim` + `accidental: truncate | reangle | slide | clear`.
Then *"the line empty space is inconsistent"* (C4 → E♯4 / G♯4 / C♯5): `truncate` measures its gap ACROSS THE PAGE, so a
steep line — still low where it stops — showed more space. ✅ `house` now uses `clear` (OURS): the line, still aimed at
the head, runs until it comes within the gap of the sign's BOX — the same space at every angle (checked in Chromium).
Then *"use ink, not bbox"* / *"no, you should not bake"*: measured on Bravura's REAL sharp, its left side is a COMB (two
crossbar tips, empty space between and under them) and each angle meets a different tooth; a box — even less the
font's cut-outs — cannot see it (box-only would still vary by ≈0.3 sp). ✅ `fonts/glyphOutline` reads the music face's
real outline from the font file at runtime (opentype.js, async, loaded once; the app re-renders when it arrives; ⛔ no
bake), and `clear` stops the line 0.3 sp before the point where, carried on, it would touch that ink. Chromium: 0.30 sp
ahead of the line at 11°, 30° and 49°.
