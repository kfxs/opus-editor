...
> ⚠️ 2026-09-19: VexFlow is removed. A bar's voice is now ours — `rendering/format/barVoice` (VexFlow's `Voice`,
> transcribed; ⛔ `STRICT` not ported, the editor never asked for it) and `rendering/format/columnFormat` (the
...
- **Do not regress the clef-beam decision:** beams stay beamed *across* mid-measure clef
  changes (see project memory / `docs/how-it-works/note-selection-hit-detection.md` companion decision).
