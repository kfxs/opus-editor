...
**🚨 SUPERSEDED IN PART — see `docs/plans/dynamic-voice-scope-plan.md`,** the *scope* a dynamic (and a hairpin) governs. It corrects this document's *"per-voice velocity step function"* and its *"voice-ready end-to-end"*: with every stamp writing `voice: 0`, a dynamic was audible to **voice 1 only**. Since that plan's P1, an absent `voice` means **ALL voices of the mark's own staff** — and the resolvers compare the STAFF too, which they never did.
...
(`docs/plans/dynamic-voice-scope-plan.md`): a mark's `voice` now means *which voices it governs*, absent =
all of its staff.
...
test written with it (`docs/plans/dynamic-voice-scope-plan.md`).
...
we chose as the default, `docs/plans/dynamic-voice-scope-plan.md`); keep a
