...
- **Reused since.** That `getModifierStartXY` override is now the shared lever for articulation X
  placement: the **articulation stem-align** feature (`docs/how-it-works/articulation-stem-align.md`) rides the
...
report the spacing feature got (docs/plans/note-spacing-plan.md §7), and the same two causes underneath —
...
- **Keyed by the member's OWN pitch id** — canonically `member.pitches[0].id`, resolved from whichever
  ...
  `docs/plans/fanned-beam-pitches-plan.md` P1, and the compartment is already id-keyed. ⛔ **Not by member
...
- **Travel across copy/paste/rebar. ✅ DONE.** Slot ids are re-minted by the `RebarEvent` stream, so
  ...
  object (docs/history/refactor-plan-2026-07-27.md Phase 4) is ALL the plumbing there is: `pasteEvents` reads
