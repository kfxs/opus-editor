...
sources) and `docs/how-it-works/above-staff-ladder.md` (where a trill sits vertically, and why we are NOT
...
3. **Outside slurs, and further from the noteheads than articulations.**
   ...
   under it, exactly as the dynamics line below is blind to a staccato dot. `docs/how-it-works/above-staff-ladder.md`
...
- **y**: clear the ink band over the trill's own columns by `0.5`, floor at `1.0` from the staff,
  ...
  its own span and nothing else. See `docs/how-it-works/above-staff-ladder.md` for why that is the whole vertical
  story here.
...
- ⭐ **`x` flips `placement`**, joining the branch that already flips a selected slur's / tie's /
  ...
  2026-08-22 the hairpin flips its side too — docs/plans/dynamics-line-and-hairpins-plan.md §"The two
...
kind-agnostic guide (`docs/plans/dynamic-offset-plan.md`), and two edits: `drawTrill` captures the
...
- ⭐⭐ **THE ARMED STAMP DRAWS A `tr` AT THE POINTER — amended 2026-08-17, his call**, and the wrong
  ...
  one tool. The ottava and the pedal followed the same day; see `docs/plans/ottava-plan.md` P5 and
...
| the above-staff ladder | `docs/how-it-works/above-staff-ladder.md` §4 — its trigger is 8va or technique text, not this |
...
- **P2 — drawing. ✅ BUILT 2026-08-13.** `engine/layout/inkBand.ts` (readers **and** the
  ...
  `TRILL_LINE` — so the LADDER (docs/how-it-works/above-staff-ladder.md §3: LilyPond's `TrillSpanner` 50 against
...
- The trill/slur COLLISION — its own plan, `docs/plans/trill-slur-clearance-plan.md`.
