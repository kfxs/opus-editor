...
feature (client #5, `docs/plans/rest-shift-plan.md`):
...
- Plumb through `pasteEvents(..., clipRestHidden)` the same way `clipRestShifts`
  ...
  *(Superseded by the `Clip` object — docs/history/refactor-plan-2026-07-27.md Phase 4: it rides on
...
skipped at draw time instead. See docs/how-it-works/pdf-export.md §4 — and note that the generalisation
...
(`docs/plans/json-io-plan.md`: report, never repair).
