...
Companion to `docs/research/tempo-marks-research.md` (findings, prior art, VexFlow survey, polytempo
...
1. **`Score.tempo` is deleted.** Tempo is resolved positionally from marks; the fallback is an
   ...
   (the `score.clef` mistake — see `docs/plans/clef-model-plan.md`).
...
- ⭐ **The ATTACHMENT GUIDE (added 2026-08-17, his call).** A selected mark draws the dashed line to
  ...
  `docs/plans/dynamic-offset-plan.md`: `drawTempoMarks` captures the endpoints onto the registry entry, and
...
- **Edit the word** (Sibelius-style): double-click → the existing `DomTextEdit` overlay
  (`TextEditController`, `docs/plans/text-editing-plan.md`). Must preserve the mark's id and leave `bpm`
...
rule, opt-in, and the tempo mark does not opt in — `docs/plans/mark-drag-and-reanchor.md`.** 🚨 The mark's own lift must come back out of that measurement first, or its "home"
...
re-base. See `docs/how-it-works/passage-selection-marks.md`.
...
tempo arm (`docs/plans/element-copy-paste-plan.md`). ⭐ Its landing rule is its own:
