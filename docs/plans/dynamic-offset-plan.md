...
- **Its far end is a PLACE** — like the hairpin's and the tempo mark's. An octave line is a
  clef-shaped statement (§4 of `docs/plans/ottava-plan.md`): it governs a REGION, every voice and every
...
3. **`scaleElement`** — a reduced staff registers in its own scaled space, so every coordinate is
   ...
   small staff — the bug class `docs/plans/staff-size-plan.md` calls "visual coords in a scaled scope".)*
...
(`measureShapeKey`, docs/history/render-performance-plan.md §7a), and this override is a textbook
...
- ⭐⭐ **…and leaving the mark's own system is a JUMP, not a walk** (his report the same day: *"it does
  ...
  - ⭐⭐ **The limit is WHERE THE MARK WOULD LOOK AT HOME** — his call after trying the obvious one:
    *"crossing the stave is not a good limit… a more organic limit vertically"*. Crossing the
    ...
    and where it would sit**. ⚠️ **Still the rule here (2026-08-30)**: the OTTAVA's drag
    now WAITS for the middle of the white space between the two staves' ink on top of it, because
    this line falls a third of the way up the gap for a mark hanging on the far side of its staff —
    but that gate is opt-in and the dynamic does not opt in (`docs/plans/mark-drag-and-reanchor.md`). ⭐ No constant anywhere in it: the gap is measured every frame (it
...
docs/history/render-performance-plan.md §12.5a — a mark drag now redraws one FAMILY against the last full
...
(docs/plans/dynamics-line-and-hairpins-plan.md P3) — so the frame that moves a letter takes the wedges down
