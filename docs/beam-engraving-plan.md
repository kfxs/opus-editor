...
| … | the beam's own **stem lengths** | `applyStemExtensions` — ⚠️ the same research as P3e (`docs/stem-length-research.md`), from the other end. ⭐ **The RULE is ours since S7b** (`engrave/beams/beamedStems`, VexFlow's transcribed exactly — every stem meets the line), so a researched change is now an edit there; ⛔ none is made yet | ⏭️ port ✅, research change open |
