...
Companion to `docs/history/render-performance-findings.md`, which measured what a keystroke costs. This doc
...
(docs/history/render-performance-research.md §7i), which attributes forced layout **by function name**.
...
(`docs/history/render-performance-research.md` §7a) — the flag was skipping the cheap half and keeping the
...
`docs/history/render-performance-research.md` §7i, where it had been recommended as the top instrument.)
...
reads)? ⭐ `docs/history/render-performance-research.md` §7 says the same split decides the fix, from the
