...
| **Choosing a colour** | Three semantic modules in `utils/`, never a stray hex: **`voiceColors.ts`** = per-voice note/rest/tuplet selection (V1 blue / V2 green / V3 orange / V4 purple); **`selectionColors.ts`** = the non-voice `INDICATOR_INK` blue, shared by the gutter, the Keypad mode arrow, and every element selection whose ink does NOT belong to one voice's notes (clef/time-sig/barline/dynamic/tempo text, and — since 2026-08-19 — the **hairpin**, the **8va** and the **pedal**, each of which governs every voice under it) — orange is reserved for voice 3, so elements must **not** select in orange. ⭐ The line is *does this ink belong to one voice's notes?*, NOT *does the model give it a `voice` field*: a `Hairpin.voice` says which stream it speaks FOR. The **TRILL** stays a voice colour, taken from the note it is anchored to (its auxiliary is a step above THAT pitch), as ties and slurs do; **`chromeColors.ts`** = window/menu/keypad neutrals. Slur-edit handles keep their own orange(open-join)/blue(true-end) language on purpose. |
