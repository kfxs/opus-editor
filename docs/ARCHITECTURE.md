...
| Ctrl-clicking a MARK into the group | `interactions/markGroupSelect.ts` — ⭐ it RE-RUNS the press chain (filtered to the six kinds `selectedItems` can hold) with a different TAIL: `pick` records instead of replacing, every drag and the editor door are no-ops. ⛔ Never a second hit-test: which mark a press lands on is `ELEMENT_HIT_ORDER`'s answer, padding, note-body guards, priority and all. `SelectionController.toggleMark` is `toggleNote`'s twin. See `docs/passage-selection-marks.md` |
| What a BOX selection drags along with its notes | `interactions/enclosedMarks.ts` — the dynamics under them and every slur / hairpin / trill / octave line / pedal FULLY enclosed by them, as `SelectionItem`s. ⭐ **The highlight is a promise about the copy**: the clip already took these (`clipboard.ts`'s `*InWindow`), so one module answers it and the highlight, Delete and the paste's own selection all read that answer. ⚠️ The rule is stated twice — here by id, there by offset — and `enclosedMarks.test.ts` asserts the two agree. See `docs/passage-selection-marks.md` |
