...
A map of the codebase for humans. For *what to build next* and how each feature was built, see
`docs/plans/` (`docs/README.md` says which of them are live). For *how the pieces fit together*, read this.
...
> **`docs/plans/spacing-model-plan.md`**, on the evidence in **`docs/research/spacing-model-research.md`** (Gould's
...
> carrying both; `MeasureLayout` asks them for a bar's width; and `rendering/format/spacingPass.ts` writes
> the x's onto the tick contexts between `format()` and `draw()`, so the tick-proportional
> softmax (VexFlow's, ported into `rendering/format/columnFormat`) no longer decides anything horizontal (⏸️ one exception, his call: a clef change after a bar's
...
> both tested.
>
> 📄 **Who decides what, and which of it to take next: `docs/history/vexflow-boundary.md`** — the full
...
> never *do we control this?*). ⚠️ 2026-09-19: VexFlow is REMOVED (`docs/history/vexflow-removal-map.md`
> S14) — nothing is decided by it any more; what it did is our own ported code, and that inventory
> is now history.
>
> 📄 **…and whether the DRAWING should move too: `docs/plans/own-engraving-engine.md`** — the 2026-08-16
> audit (⚠️ 2026-09-19: VexFlow is now removed entirely — `docs/history/vexflow-removal-map.md` §9). The measurements then (all 38 `vexflow` imports lived in ONE directory; `Stave` is a coordinate
...
>
> **There is no UI framework.** Vue was removed (docs/history/remove-vue-plan.md); the
...
│      ClipboardController .... copy / paste — the MUSIC or one  │
│                               ELEMENT (+ clipboard.ts,         │
│                               elementClipboard, pasteAnchor)   │
...
│      NoteEntryCoordinator .. click → beat, collision, commit   │
│      models/ScoreModel ..... THE data model (see glossary)     │
│      models/{clef,tuplet,rebar,slur,override,mark,voice}Ops     │
│      models/{deleteNote,convertToRest,spanningNote,            │
│        entryOverwrite,durationChange,tupletEntry,restFill,     │
│        slotPlacement,measure,tie}Ops — the entry + repair ops  │
...
│      rendering/format/spacingPass ... WHERE each column goes — the     │
...
│      audio/repeatPlan ...... THE PLAY ORDER — a repeat as legs  │
├─────────────────────────────────────────────────────────────┤
...
(`docs/history/refactor-plan-2026-07-27.md`):
...
- **The score layer was not fenced against `engine/layout/` or `engine/fonts/`** — the docs said
  the core knows no engraving, and `barlineOps` imported `layout/barlineSign` (and through it the
  font metrics) to ask which sign stands at a boundary. What a score IS may not depend on how much
  room or ink it takes. The model-only half — `signAtBoundary`, `wingsAllowed`, the
  `BarlineSignKind` type — is now `engine/models/boundarySign.ts`, and both folders are in the
  score layer's restricted imports. ⚠️ `wingsAllowed` is a table there, because the core cannot
  read the sign's parts; `barlineSign.test.ts` holds it to `hasThickLine`, which does.
...
- **`src/engine/**` as a whole was not fenced against `interactions/`** — only its
  ...
  the vocabulary it wants — `engine/rendering/ghosts/ghostTypes.ts` — and the editor translates
  into it (`interactions/state/toolGhost.ts`), which is the same shape as the `RenderProbe` fix
  above.
...
`interactions/clipboard/clipboard`).
...
ratchet the score layer has, for a different reason (docs/plans/font-metrics-plan.md F4).
...
`rendering/painter/musicFontReady.ts` exists to survive — ⚠️ a race that is real and still costs the
*drawing* a deferred first render, because **the drawing half has no metrics table and measures every
glyph off a canvas** — VexFlow's way, kept by the port in `rendering/painter/glyphPainter`
(`reference_vexflow_measures_glyphs_at_render_time`). One `measureText`
...
- **`engine/fonts/` may not import `engine/rendering/*`** (nor `vexflow` — which no file anywhere
  ...
  module that stays unchanged when the drawing moves (docs/plans/own-engraving-engine.md P2/P3).
- **`engine/layout/` may not import `engine/rendering/*`** — the arrow is rendering → layout, never
  ...
  `PAGE_GAP_PX` — with their specs (docs/plans/code-shape-plan-2026-09-19.md Phase 2.3).
...
(docs/history/remove-vue-plan.md), so the boundary now separates *the app shell* from
...
npm run lint:boundary     # no framework anywhere below App.ts; dev/ out of the engine;
                          ...
                          # layout/ + fonts/ + paint/ fenced against the DOM;
                          # ⛔ `vexflow` refused in EVERY file, specs included (S14)
...
⭐⭐ **`lint:paint` was a PROGRESS BAR, and it reached zero** (`docs/plans/own-engraving-engine.md` P1b;
S13b of `docs/history/vexflow-removal-map.md`). The engine draws through its own
`engine/paint/DrawContext`, and behind it on the page is OUR `rendering/painter/SvgPainter`. What needed
...
`docs/history/refactor-plan-2026-07-27.md` Phase 0c: **a comment asserting a fact about the
...
> `ScoreModel` or `ScoreRenderer` — nor a per-kind slice to `PaletteController`,
...
This is the standing convention that `docs/history/refactor-plan-2026-07-27.md` exists to establish,
...
the 211 commits over the next nine days it grew back by 846, while `ScoreRenderer` grew by
...
Phase 6 then cut `ScoreRenderer` 5,491 → 3,744 (the fan pass and the ghosts moved out whole)
...
**Why the clause now names eight files and not three** (`docs/history/modularity-plan-2026-07-28.md` §3,
...
| a DRAG gesture | `interactions/drags/<kind>.ts` — `begin<Kind>Drag(host, …)` returns a `Gesture` whose state lives in its closure. A mark that walks the music is a ROW of a shared frame, ⛔ not a frame of its own: `drags/bodyDrag` (free), `drags/heldDrag` (it latches on notes — the hold) | ⛔ none in `MouseController`, and none in `chain.ts`: the kind's element module arms it, `deps.arm(door => begin<Kind>Drag(door.host, …), event)`. A span's SQUARE adds a row to `drags/markEnd`'s table |
| what the ARROWS do to a selected element | `interactions/elements/<kind>Keys.ts` — `{ nudge, reset, reanchor, cycle }`, each optional. ⭐ A span mark writes NO module: its row is `keys: spanMarkKeys('<kind>')`, read off `SPAN_MARK_TOOLS` | `keys:` on the kind's `ELEMENT_SPECS` row — ⛔ no closure and no `\|\|` link in `shortcutWiring`. `chain.test.ts` pins WHICH kinds answer each verb, because a lost row is silent |
...
packaging goal (`docs/history/refactor-plan-2026-07-27.md` §Context) changes is the **consequence**:
...
once a hub has been emptied (docs/plans/code-shape-plan-2026-09-19.md Phase 2.1 says why). The two
...
but their count only falls (docs/plans/code-shape-plan-2026-09-19.md, Phase 4.4).
...
there, and it exists because the span-mark pass (`docs/plans/span-mark-family-plan-2026-08-24.md`, four
...
| Re-barring / paste — the region-rewrite pipeline (capture → relay → materialize → restore) | `engine/models/rebarOps.ts` (free funcs over `score`; ScoreModel delegates). ⭐ No callback bundle since the code-shape plan's 4.3: what it needs of the model — `measureOps`, `restFillOps`, the three dangling-reference sweeps — it imports |
| Clef / tuplet mutations (sibling delegated sub-APIs) | `engine/models/clefOps.ts` / `tupletOps.ts` |
| Deleting a SELECTION — what silence a cleared region leaves | `engine/models/clearOps.ts` (free funcs over `score` + a two-callback `ClearRangeDeps` — the raw delete and `deleteNoteWithRepair`, the two that need the MODEL; `MusicEngine.deleteNotes` is the command). ⭐ The region is emptied and the METER fills it: remove every slot first, then refill each touched bar **once** (`utils/restFill`), so clearing half a 4/4 bar is one half rest and not one rest per slot. ⛔ Only the region — a rest outside the selection bounds the hole and is never swallowed. A single note is a range of one. See `docs/plans/clear-range-plan.md` |
...
| Placing a note — WHERE a click lands (grid snap, the nearest rest, the clicked staff), collision, the ORDER of the steps, the commit | `engine/NoteEntryCoordinator.ts` |
| …and what the entry DOES to the score (code-shape plan, Phase 4.2 — one module per subject, ⛔ not one `noteEntryOps`) | `engine/models/spanningNoteOps.ts` (a note past the barline = a tied chain; the next bar's erosion, which spares the slot's OWN continuations) · `entryOverwriteOps.ts` (⚠️ TWO rules on purpose: the keyboard asks whether INTERVALS overlap, the mouse whether a note STARTS inside) · `durationChangeOps.ts` (`changeNote` → `{ note, commit }`) · `tupletEntryOps.ts` (build / apply / `tupletFitsBar`; the keyboard's clamp, the mouse's fill pointer) |
| Deleting ONE note / silencing a slot — and the repair the bar is owed (ties, slurs, the replacement rest, the voice collapse) | `engine/models/deleteNoteOps.ts` / `convertToRestOps.ts`. The facade keeps the undo LABEL and the one `mutate` |
| The REST FILL (every voice of every staff sums to its bar) · placing a slot (the rests it evicts, their ties, the hidden flag) · making a measure | `engine/models/restFillOps.ts` · `slotPlacementOps.ts` · `measureOps.ts` |
| What a click/drag/pan *does* | `interactions/controllers/MouseController.ts` — the press's pre-steps, the pan, box-select, and the ONE gesture in flight. ⭐ Which ELEMENT a press picks is not here: it is `interactions/elements/` and the chain it loops over. ⭐⭐ And what a DRAG does is not here either: `interactions/drags/` — the controller holds a `Gesture`, hands it every move (`move` may answer `false`, "not mine yet", for a press still in its dead zone) and ends it on the release, wherever that happens. It keeps no per-gesture state |
| What an ARROW KEY does to the selected element | ⭐ The element's own row: `ELEMENT_SPECS[kind].keys` (`interactions/elements/keys.ts` is the contract). `nudge` / `reset` move INK (plain = fine, `Ctrl` = coarse, `Ctrl+Backspace`); `reanchor` moves it through the MUSIC (`Ctrl+Shift+←/→` — a model write, usually audible; on a span the ARMED SQUARE says which end); `cycle` is `Tab`. `dx`/`dy` arrive in SCREEN staff-spaces, so a kind with its own convention converts in its row (the tempo's outward `y`). `shortcutWiring` asks the row first and falls through on a decline — what is left in its chains keys off the NOTE selection (a rest's shift, note spacing, bar width, the note offset) |
| Letter-key note entry, chord/rest entry | `interactions/controllers/KeyboardController.ts` |
| The selection set, multi-select, keyboard nav | `interactions/controllers/SelectionController.ts` |
| Ctrl-clicking a MARK into the group | `interactions/state/markGroupSelect.ts` — ⭐ it RE-RUNS the press chain (filtered to the kinds `selectedItems` can hold — `MarkKind`) with a different TAIL: `pick` records instead of replacing, every drag and the editor door are no-ops. ⛔ Never a second hit-test: which mark a press lands on is `ELEMENT_HIT_ORDER`'s answer, padding, note-body guards, priority and all. `SelectionController.toggleMark` is `toggleNote`'s twin, and both start by ABSORBING a single-element selection into the set (`absorbElementIntoSet`) — a plain click on a note lands in the set, a plain click on a mark lands in `selectedElement`, and a Ctrl-click grows what you have either way. See `docs/how-it-works/passage-selection-marks.md` |
| What a BOX selection drags along with its notes | `interactions/clipboard/enclosedMarks.ts` — the dynamics and tempo marks under them, every slur / trill they hold, and every hairpin / octave line / pedal that BEGINS in them (⭐ a span belongs to where it begins — the model's own filing rule — and its extent travels verbatim; ⛔ never truncated), as `SelectionItem`s. ⭐ **The highlight is a promise about the copy**: the clip already took these (`clipboard.ts`'s `*InWindow`), so one module answers it and the highlight, Delete and the paste's own selection all read that answer. ⚠️ The rule is stated twice — here by id, there by offset — and `enclosedMarks.test.ts` asserts the two agree. See `docs/how-it-works/passage-selection-marks.md` |
| Copy / paste | `interactions/clipboard/ClipboardController.ts` dispatches; ⭐ it holds the MUSIC **or** ONE element, never both — the last Ctrl+C is which. WHAT travels: `clipboard.ts` (notes + everything that rides a lane; ⭐ **silence is content** — the clipboard is the one caller that asks `flattenRegion` for `keepRests`, so a copied rest travels as an event in the shape you see (a rebar still treats one as a gap to re-shape), and the clip is usable on its SPAN rather than its event count; 🚨 `RebarEvent.written` carries the authored duration+dots so a dotted quarter does not come back as a quarter tied to an eighth — it wins only where the event survives WHOLE, and only when it describes the length; ⭐ the marks are the INTERSECTION of what the window can carry and what the SELECTION holds — `MarkFilter`) / `elementClipboard.ts` (one mark — a dynamic or a TEMPO mark: its text, how it reads, and what it sounds; ⛔ **no id**, so a paste mints a new one and the copied mark's override nudge stays behind). ⭐ The anchor module says where the selection POINTS; what a kind may do with that point is the kind's own rule and lives there — a tempo mark lands on the nearest ONSET at-or-after it (`tempoOps.tempoAnchorAt`, so the model agrees with `TempoLayout.anchorX`; ⛔ never a barline) and REPLACES the one mark that beat may hold. WHERE it lands: `pasteAnchor.ts` — notes → the earliest one's exact slot, an element → the nearest anchorable point (⭐ a BARLINE means the NEXT bar's downbeat, never the line itself), nothing → **null** and the click decides. ⛔ Model-only: it reads the score, so a culled bar anchors as well as a drawn one. See `docs/plans/element-copy-paste-plan.md` |
| Which tool/duration/accidental is armed | `interactions/controllers/PaletteController.ts` |
| Adding a 9th marking tool (clef/dynamic/stamp/…) | `EditorState.MarkingTool` + build: the compiler names every site — see `docs/how-it-works/marking-tools.md` |
| Adding a selectable on-score element (a new thing a click can pick) | ⭐ ONE new module: `interactions/elements/<kind>.ts` (its hit-test AND how it paints — plus, if it drags, the gesture it arms from `interactions/drags/`, and, if the arrows move it, its `keys`), a row in `ELEMENT_SPECS` and — if a press can land on it — a position in `ELEMENT_HIT_ORDER` (`elements/chain.ts`; the ORDER is the content, and `chain.test.ts` pins it). Then `EditorState.SelectedElement` + build: `assertNeverElement` still names the two sites that stay switches, Delete (`shortcutWiring`) and the Properties report (`selectionSnapshot`) |
| How notation is drawn to SVG | `engine/rendering/ScoreRenderer.ts` — the pass ORDER and the per-bar work. One family per module beside it, each a free function over `RenderPass`: `TieRenderer`, `SlurRenderer`, `DynamicsLayout`, `TempoLayout`, `FanPass`, `GhostRenderer`. ⭐ A new drawn family joins that list; it does not join the renderer |
| **The surface anything is drawn on** | ⭐⭐ `engine/paint/DrawContext.ts` — OUR type, 20 primitives, importing nothing, and implemented on the page by OUR `rendering/painter/SvgPainter` (S13b — VexFlow's `SVGContext` transcribed; ⚠️ its markup is a contract). A pass takes `RenderPass.context`. ⛔ Reach for `RenderPass.painter` only to read the PAGE back (`.svg`) or rewind the ambient ink (`state`/`attributes`). ⛔ `paint/` may not import the DOM, `vexflow`, or `models/` |
| **HOW BIG A DRAWN THING IS** (its ink box) | ⭐⭐ `engine/scene/sceneBox.ts` — computed FROM THE SCENE: a rect from itself, a path from its ops (a cubic's true extrema), a glyph from `fonts/GLYPH_BOXES`, a group as the union of its children **with the caller choosing which count**. ⛔ It never guesses: an unmeasured glyph makes the whole box `null`. `rendering/painter/sceneInk` supplies the pt→px dialect inherited from VexFlow (`scene/` and `fonts/` may not know it). ⚠️ **Nothing consumes it yet** — `ElementRegistry` still stores the boxes the port kept EXACT from VexFlow's (`getBoundingBox` & co., ours now); that switch is P6b (`docs/plans/own-engraving-engine.md` §5 P6) |
| **WHAT A RENDER ACTUALLY DREW** (a geometry assertion without a browser) | ⭐⭐ `engine/scene/` — `ScoreRenderer.recordScene(fn)` tees a `SceneRecorder` onto the real painter, so the page paints as normal and hands back a `Scene`: plain typed values (rect / path / text / pointerRect / group), ⛔ no DOM, ⛔ no VexFlow. Read it with `scenePrimitives` / `sceneGroups` / `walkScene`. ⚠️ It holds what was drawn through the pass's `context` — ⛔ **NOT** what is drawn straight on `RenderPass.painter`. Since S13b everything on the page is painted by OUR code (the painter is our `SvgPainter`; nothing of VexFlow's paints itself), and the note's ink (P3), the beam's lines (P4), the staff and its header (P5) and **every slur and tie arc** (U1) reach the scene; a draw still handed the painter itself (e.g. the tuplet's `draw(this.context!)` in `ScoreRenderer`) does not, and handing it the pass's surface is engine work |
| **A GROUP of drawn ink** (a bar's own `<g>`, a mark's, a ghost's) | ⭐⭐ `engine/paint/DrawGroup.ts` — what `DrawContext.openGroup` gives back, wrapped by `rendering/painter/svgDrawGroup`. Five capabilities: `setPlacement` (⭐ an **affine**, `paint/Affine.ts` — ⛔ never "a scale" or an `(x, y)`), `inkBox` (⚠️ **before** the placement, and **null** in jsdom = *nothing measurable drew*), `discard`, `tag` (the group), `tagLast` (the primitive just drawn). ⛔ `node()` is the escape to the real `SVGGElement` — only for handing ink to the EDITOR to highlight, or recolouring through the page; `npm run lint:paint` counts it. 🚨 A group's `transform` is READ AS TEXT by `moveMeasureGroup` and four specs, so `svgDrawGroup` emits the SVG shorthand where it is exactly equivalent |
| **Stamping one music glyph** (a repeat dot, a `tr`, a key-signature sign, a grouping brace) | ⭐⭐ `engine/rendering/painter/glyphPainter.ts` — `drawGlyph` / `measureGlyph` / `drawTextRun`, and ⛔ **never measure or stamp a glyph in your own file**: it is the ONE place a glyph's face is resolved, measured and stamped (`docs/plans/own-engraving-engine.md` P1a; no VexFlow since S13a — faces from `engine/fonts/fontCategories` + `fontFace`). ⚠️ Sizes are POINTS (`./drawnFontSize`), the `y` is the BASELINE, and a returned width of 0 means *no measurable font* (jsdom), which callers already treat as "draw nothing". 🚨 The tag argument is not a label — it resolves the FONT via `fonts/fontCategories` (VexFlow's `Metrics` categories, ported), so ⛔ never name a tag after one of those categories (`TempoLayout` and `ScoreTuplet` once kept their own VexFlow `Element`s for this reason; both stamp through this module now) |
| A staff that is not STRAIGHT (a circle, any path — eye music) | `engine/engrave/staff/staffSpine.ts` — a `Spine` answers *where am I at distance s, and at what angle* and its inverse; a new shape is a new ROW there. `spineLines.ts` draws the lines FROM it; `engine/rendering/eye/` places rigid blocks ON it (`placementAt` — one affine, ⛔ never a deformation) and `spineScore.ts` reads the open score. ⚠️ Shown only by `__spine.*` in a panel of its own; the real render path asks no spine yet — `docs/plans/bent-staff-plan.md` §4 is the decision list |
| A cursor GHOST (the translucent preview an armed tool shows) | `engine/rendering/ghosts/GhostRenderer.ts` — a `draw*Ghost`, a `ToolGhost` member (`ghostTypes.ts`), a `GHOST_DRAWERS` row, and a case in `interactions/state/toolGhost.ts`. ⭐ A ghost with any drawing of its own gets its OWN module beside it (`FanGhost`, `TrillGhost`, `OttavaGhost`, `PedalGhost`, `HeaderSignGhost`, `MarkGhost`) and reuses ITS PASS's own draw function, so a preview cannot become a different glyph from the engraved mark. ⚠️ The payload is ENGINE-owned, never the editor's `MarkingTool`: `lint:boundary` fences `src/engine/**` off from `@/interactions`. `ScoreRenderer.ghostOverlay` frames every one: take the last ghost down, refuse if there is no page. ⚠️ A ghost's class must be in `GHOST_GROUP_SELECTOR` or it is never removed and smears a trail — and in `e2e/harness.ts`'s own copy of that list, or the browser test reads an empty page |
| **THE INTERPOLATING WALK** — ←/→ moving a mark's INK, and the anchor going with it | `interactions/walks/markWalk.ts` — the identity, the arrival test, the latch, `crossWithoutArrival`; `interactions/walks/markBreakWrap.ts` — the SYSTEM BREAK, and `systemInkAt`. Six families PORT them (`dynamicWalk`, `tempoWalk`, `hairpinWalk`, `ottavaWalk`, `pedalWalk`, and the trill's own FOLD), each with a `*Lane` module holding the drawn x's the mouse and the keyboard both measure. ⭐ **A seventh writes a port, ⛔ never a copy and ⛔ never a `kind` switch.** 🚨🚨 **A system is NOT named by its staff's top-line y** — the first line of every page shares it, which made the wrap measure another sheet's bar (a gap of 128 sp, a mark landed at −117): `systemInkAt` names it by the CONTIGUOUS RUN of bars sharing the row. ⭐ A crossing needs a RE-BASE the page limit does not judge, or a refused re-base leaves the anchor ahead of the ink and the next press crosses again. ⭐ A press whose ink a limit refuses still steps the ANCHOR (`crossWithoutArrival`) — the offset goes HOME, ⛔ not re-based by the gap. See `docs/plans/ottava-plan.md` §P11 and `docs/plans/pedal-plan.md` §P6 |
| A GUIDE from a selected element to what it is attached to (the dashed "anchor line") | `interactions/elements/anchorGuideLine.ts` draws it; the RENDER measures it. ⭐ Kind-agnostic — **adding a kind is two edits, neither in the guide**: the pass that draws the element registers `guides: [{ from, to }]` on its `ElementInfo` (`from` on the element's own INK, `to` on what it hangs off), and the kind's row in `ELEMENT_SPECS` calls it from `highlight`. Six kinds (dynamic, tempo, trill, hairpin, ottava, pedal), and the one question a new one answers is whether it attaches to a NOTE (it is computed from that pitch) or to a PLACE (it governs a region) — the table is in `docs/plans/dynamic-offset-plan.md`. ⭐ A SPAN registers per system fragment and the drawer reads every entry under the id, so each fragment's line is in its own system's coordinates. ⚠️ Ink, not box: see `rendering/marks/dynamics/dynamicMarkInk.ts` for why a bbox corner reads as blank space over a `p`. 🚨 A new coordinate field on a registry entry must be taught to THREE functions (`offsetElement`, `scaleElement`, `shiftById` — where only `from` ends move); `docs/plans/dynamic-offset-plan.md` carries that checklist, the audit that found `segmentEndpoints` missing from two of them, and MuseScore's dispatch table for the kinds still to come |
| WHERE a cursor ghost sits, and whether a tool ALSO takes the blue caret | `engine/rendering/ghosts/ghostCursor.ts` — ⭐⭐ ONE position for every sign-shaped ghost, taken from the accidental's (left of the pointer, centred on its line — his reference, 2026-08-17). ⛔ **Never derive a ghost's offset from where its mark is engraved**: a ghost is a sign FOR THE USER, and two bugs in one day came from forgetting it (`Ped.` dropped below the pointer, where the arrow covered it; `8va` above and `8vb` below, so the eye re-found it on every switch — `docs/plans/pedal-plan.md` §7). And a tool that draws a ghost must NOT be in `scoreCursorClass`'s `cursor-place` list: the TOOLS in it are exactly `toolGhost`'s `null` cases. ⚠️ One entry there is not a tool — an armed PASTE (2026-08-19), which draws nothing at the pointer either since the dashed caret was dropped (`docs/plans/element-copy-paste-plan.md`) |
...
| What a bar-width gesture may do (slopes, floors, the ceiling) | `engine/layout/barWidthRoom.ts` — a PURE function of the casting-off + the stored stretch + the view mode + the surface + one measured slack, so `docs/plans/bar-width-plan.md` §4–§5 can be stated in a unit test |
...
| Scroll / zoom / viewport | `engine/ViewportModel.ts` (pure), `interactions/controllers/ViewportHost.ts` (DOM) |
| Playback / audio | `engine/audio/PlaybackEngine.ts` (clock + scheduling) |
| **What a REPEAT means to playback** | `engine/audio/repeatPlan.ts` — ⭐⭐ the performance is a list of **LEGS**, ⛔ never a re-ordered score: the same bar sounds more than once, so bar↔time stops being one-to-one and every consumer of that mapping breaks at once (the schedule, the auto-stop, the progress bar, the playhead). A leg is a contiguous run of bars carrying **where in SCORE beats** it comes from and **when in PERFORMANCE seconds** it happens; ⚠️ keeping those two clocks apart is the whole of the care, and a leg's duration is the DIFFERENCE of two `beatsToSeconds` lookups rather than a beat-length × one rate (a repeated passage may straddle a tempo change). ⭐ A score with no repeats is ONE leg, and every arithmetic downstream collapses to what it was. ⛔ `barlineOps` gained no caller: the SIGNS are the music, the ORDER is one performance of them — which is why a checkbox (Play ▸ Play Repeats / the dev shell's 🔁, sharing `bus.playRepeats`) can switch it without touching a score, and why it is not a `Score` field. Voltas / D.C. / D.S. are NOT built — every format holds a volta as a *container of measures*, never a barline attribute, so they are inputs to that same walk |
| **What a BARLINE is made of** | ⚠️ WHICH sign a boundary carries is the score's — `engine/models/boundarySign.ts` (`signAtBoundary`, the precedence). What it is made of: `engine/layout/barlineSign.ts` — the ONE owner of a sign's strokes, dots and extent, in staff spaces from the boundary (four consumers need the same number: the pass that draws it, the width the bar reserves, the hit-box, the highlight). ⭐⭐ Its `SignHalf` rule — **the divider is `shared`, everything left of it belongs to the bar the line ENDS, everything right to the bar it OPENS** — is read off the geometry and answers three separate questions with no second table: which half of a `:||:` a selection lights, whether a sign has a `start` half (⇒ it earns a `repeatStart` hit-box), and where the WINGS go. Drawn by `engine/rendering/staff/BarlineRenderer.ts`: ⭐ **it draws every barline that ends a bar**; the line that OPENS a system is the stave's own (`engrave/staff/openingBarline`, via `EngravedBarline` — VexFlow's until P5b) |
| **Whether a barline runs INTO THE NEXT STAFF** | `engine/models/barlineJoin.ts` — `barlineJoinsBelow(score, staffId, measureNumber)`, the ONE reader, over `StaffInfo.barlineJoinBelow`. ⭐⭐ **The BOUNDARY is in the signature although nothing varies by it yet** (`resolveStaffSize`'s arrangement, `void`ed parameter and all): the contemporary per-boundary mix then changes the lookup *inside this function* and not one caller — *what forecloses a future is a READER THAT ASSUMES, never a field that is missing*. ⛔ Absent = **not joined** (his call: what joins "all barlines" is the reach of the GESTURE, not the state a score starts in), so the feature is inert until asked for. The ink is `engine/rendering/staff/barlineGap.ts`, a module of its own for a reason that is not tidiness: 🚨 **a line crossing two staves belongs to neither SCALE GROUP**, so it may not be drawn inside `inStaffSpace` and takes the SCORE's staff-space — `drawSystemConnector`'s decision for the line joining the same two staves, and MuseScore's `Sid::scaleBarlines = false`. 🚨 Its y comes from the PLACEMENTS: ⚠️ the connector beside it reads its stave directly and is still correct only because `if (multiStaff && plan.isFirstInLine) return` never lets a system's opening bar be translated — a gap segment is drawn at EVERY boundary and has no such shelter. See `docs/plans/barline-join-plan.md` |
...
| All editor UI state | `interactions/state/EditorState.ts` |
| A keybinding | `shortcuts/ShortcutConfig.ts` |
| Starting PLAYBACK | `p` (`togglePlayback`) — Sibelius's own key for it, and like Sibelius it plays **from the selection**: `interactions/controllers/playbackStart.ts` answers which bar, ⭐ with a selected BARLINE meaning the bar that starts AFTER it (the line *ends* bar N) and a group meaning its EARLIEST member by position, not by click order. Starting **clears the selection** — during playback the thing you are attending to is the music, and a stray key must not edit the note you left picked. ⭐ `Escape` stops too, ahead of everything else it means. MuseScore spends `p` on its piano-keyboard panel; ⚠️ Space is not available to us, it is note entry's typewriter key. The shortcut calls the SAME `App.togglePlayback` the dev shell's ▶ button runs — a second way to press it, never a second implementation |
| **Playing from somewhere other than the top** | `PlaybackEngine.seekToMeasure` sets the bar; `play()` turns it into an **origin shift** via `playableOverPlan` (`audio/playbackSchedule.ts`, over the play order): drop every note attacked before it, move the rest earlier by the seconds that bar sits at, and set `playbackStartTime = now − startSeconds` so the position loop — which measures from the top of the score — needs no notion of a seek at all. The auto-stop timer subtracts the same offset. ⭐ Since repeats play (§7), "from bar N" means the **FIRST** time it comes round — ambiguous otherwise, and the answer Sibelius, Dorico and MuseScore all give. ⚠️ For months `seekToMeasure` set a field **nothing read**: it moved the position READOUT and not one scheduled note, so every play began at bar 1. It cannot be caught by a unit test (`play()` needs an AudioContext), which is why the decision lives in a pure function that has one |
| What a NUMPAD key does | `windows/keypad/keypadLayouts.ts` — **not** ShortcutConfig, which binds all 16 pad keys to one `keypadKey` action. The pad is the Keypad panel: a key presses the cell under it *on the page that is showing*. See `docs/how-it-works/keypad.md` |
...
| Which notes are beamed together | `utils/beaming.ts` (pure — a run of bars, each with its own `MeterInfo`, → index groups; one bar is a run of one). The per-note override lives on the NOTE (`Chord.beam`), not in `engravingOverrides`. See `docs/how-it-works/beaming.md` |
| A beam that crosses a BARLINE | `rendering/beams/CrossBarBeams.ts` decides which barlines are open (bounded by the system break and by any unpainted bar); the bar gives its joined notes a **placeholder** beam, and the one real `Beam` is drawn in a post-measure pass **outside both measure groups**, like a tie. It rides both bars' `measureShapeKey` and pins them as span anchors. See `docs/plans/cross-barline-beaming-plan.md` |
| How many beam LINES join them (6 sixteenths subdivided 3+3) | `Chord.secondaryBreak` — a SEPARATE field from `Chord.beam`, not a sixth `BeamMode`: which notes are beamed and how they are subdivided are independent statements. Drawn with `EngravedBeam.breakSecondaryAt` (VexFlow's `Beam.breakSecondaryAt`, ported); the index translation is `secondaryBreakIndices` in `utils/beaming.ts`. See `docs/how-it-works/beaming.md` |
| **How LOUD a note plays, and WHICH voices a dynamic governs** | `utils/dynamics.ts` owns the meaning axis (glyph run → level → velocity, and the two resolvers), `utils/dynamicScope.ts` owns the SCOPE: ⭐⭐ **an absent `Dynamic.voice`/`Hairpin.voice` means EVERY voice of the mark's own staff** — the inverse of `utils/lanes`' "absent = the first one", and true for these two kinds only, so ⛔ `voiceOf()` must never be called on them. ⭐ Two predicates, and which one a reader may ask IS the rule: `governsSlot` (staff **and** scope) answers *who gets louder* and *what hangs off this note*; `onSameStaff` (the staff, every voice of it) answers *where the mark may stand* — the walk, the drag, the drawn anchor, the wedge's tips. Fusing them made a voice-2 dynamic unmovable in a bar voice 2 is silent in. ⚠️ `resolveChordLevels` compares the STAFF too, which it never did before 2026-08-19 — a staff-2 `p` used to govern staff-1's voice 1. ⛔ A hairpin still makes NO sound: scope says which voices a wedge *will* govern, and the ramp is not built. See `docs/plans/dynamic-voice-scope-plan.md` — and `docs/plans/playback-semantics-plan.md` for where this is all meant to go (⏭️ RECORDED, not scheduled): the schedule should speak in **pitch + a dynamic value 0..1**, an INTERPRET step should convert to MIDI/velocity, and a crescendo is realised differently on a sustaining instrument than on a decaying one |
| A FANNED (feathered) beam — one note played and drawn as many | `Chord.fan` is the ASSERTION ("play this note as six, accelerating"); the RHYTHM is a projection from `utils/fannedBeam.ts` (`fanMembers`), read by BOTH the drawing (`rendering/beams/FannedBeam.ts`) and the playback so they cannot disagree. The slot keeps its own duration — one event, indivisible, so no re-tile can break the group. Only the PITCHES are stored (`FanMark.members`, member 0 IS the slot's own chord), because a pitch cannot be derived; `normalizeFan` is the one function allowed to keep them in step with `count`. ⭐ Its WIDTH is no longer its own question: each member is an ordinary COLUMN at its own exact beat (`fanMemberBeats`), so the bar asks for the sum of what those durations earn and each gap is the spacing rule applied to that member. That deleted `fanColumns`, `fanRoom.ts` and three more constants — see `docs/plans/spacing-model-plan.md` §P5. Also `docs/plans/fanned-beams-plan.md` + `docs/plans/fanned-beam-pitches-plan.md` |
| Which accidental SIGN a note displays | One forward walk — `displayedAccidentals` (`utils/accidentalState.ts`), read by `NoteBuilder` for the notes (`EngravedNote`) AND by the fan renderer for its hand-drawn member heads, so the drawing can never invent a second rule. Its query twin `prevailingAlterations` answers "what is in force at this beat?" for the palette and the selection. ⚠️ Scope is the CALLER's: pass one lane's slots, and use `measureAccidentalNotes` (not `getMeasureNotes`) where fanned members must count — they alter the bar like any other note |
| **Choosing a colour** | Three semantic modules in `utils/`, never a stray hex: **`voiceColors.ts`** = per-voice note/rest/tuplet selection (V1 blue / V2 green / V3 orange / V4 purple); **`selectionColors.ts`** = the non-voice `INDICATOR_INK` blue, shared by the gutter, the Keypad mode arrow, and every element selection whose ink does NOT belong to one voice's notes (clef/time-sig/barline/tempo text, and — since 2026-08-19 — the **8va** and the **pedal**, each of which governs every voice under it) — orange is reserved for voice 3, so elements must **not** select in orange. ⭐ The line is *does this ink belong to one voice's notes?*, NOT *does the model give it a `voice` field*. The **TRILL** takes a voice colour from the note it is anchored to (its auxiliary is a step above THAT pitch), as ties and slurs do; and the **DYNAMIC** and the **HAIRPIN** take **either**, ASKED OF THE DATA (`markSelectionColor`, `docs/plans/dynamic-voice-scope-plan.md` P2): they are the only kinds carrying a real SCOPE, so a mark governing the whole staff takes this blue and one narrowed to a voice takes that voice's colour. ⚠️ That could only become derived once absent `voice` stopped meaning voice 0; **`chromeColors.ts`** = window/menu/keypad neutrals. Slur-edit handles keep their own orange(open-join)/blue(true-end) language on purpose. |
...
can `registerFont`, and the injection point would be `rendering/painter/glyphPainter`'s shared measuring
canvas (VexFlow's `Element.setTextMeasurementCanvas`, which the port did not keep as an API). Even then Cairo/FreeType's `actualBoundingBoxAscent/Descent` will not match Skia exactly, so
...
`docs/history/refactor-plan-2026-07-27.md` Phase 5, as the prerequisite for restructuring the renderer in
...
- `e2e/harness.html` + `e2e/harness.ts` boot the **engine alone** — no `App`, no controllers, no dev
  ...
  jsdom. One engine per page, deliberately: the renderer reaches back for what it drew with document-wide
  `getElementById` (VexFlow's `getSVGElement` habit, kept by the port), so a second score makes those
  ids ambiguous.
...
> (`docs/plans/spacing-model-plan.md`) made a bar's width the DURATION rule summed over its columns, with
...
**3. Does it SPAN bars?**
Yes → it must be a **span anchor** (`ScoreRenderer.spanAnchors`). Otherwise culling removes the bar
...
| **voice-ready** | The data model carries up to four voices per staff, and they are entered, drawn and played (`docs/plans/multi-voice-plan.md`; rests placed by `multi-voice-rest-position-plan.md`). The word survives in older docs from when only the data shape existed: a feature that landed "voice-ready" took a `voice` from the start. ⚠️ Absent means the FIRST voice (`utils/lanes.voiceOf`). |
...
| **range clear** | What Delete does to a selection: the slots go and the **meter** fills the time they held (`engine/models/clearOps.ts` → `utils/restFill`), rather than each slot leaving a rest of its own length. Only the cleared region is refilled — rests outside the selection bound the hole. A single note is a range of one. Distinct from **silence** (`convertToRest`, the `0` key), where the length is authored and is preserved exactly. |
| **tie-split / spanning note** | A note longer than the remaining bar is split at the barline into a chain of tied notes (current measure remainder + next measure(s)). One primitive, `placeSpanningNote` in `engine/models/spanningNoteOps.ts`, behind two callers: a fresh head (`addSplitNoteWithTie`, note entry) and a reused one (`splitExistingNoteWithTie`, a duration change); a chord crosses with `splitChordWithTie`. |
...
| **measure rest** | A whole-bar rest whose duration is the nominal `'w'` meaning "fill the bar", not a literal whole note — must not be inherited as a real `'w'` duration in non-4/4 bars. The GLYPH it is drawn with is the bar-rest style's (`layout/barRestStyle`: a breve rest from 8 quarters by default), asked through ONE answer, `restVoicePlacement.restDrawnDuration` — ⛔ never a `'w'` assumed at a draw, placement or ink site (docs/plans/other-durations-plan.md P4). |
| **cautionary** | A courtesy clef/time-signature drawn at a line break to warn of an upcoming change. Handled in `ScoreRenderer` (`chooseVoiceMode` / cautionary logic) — legitimately complex; leave it. |
```
