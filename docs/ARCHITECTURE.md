...
| **WHAT A RENDER ACTUALLY DREW** (a geometry assertion without a browser) | ⭐⭐ `engine/scene/` — `VexFlowRenderer.recordScene(fn)` tees a `SceneRecorder` onto the real painter, so the page paints as normal and hands back a `Scene`: plain typed values (rect / path / text / pointerRect / group), ⛔ no DOM, ⛔ no VexFlow. Read it with `scenePrimitives` / `sceneGroups` / `walkScene`. ⚠️ It holds what OUR primitives drew — ⛔ **NOT** what a VexFlow object paints itself, which still goes through `vexContext`. ⭐ That list is now short: the fan's own heads and the modifiers a `StaveNote` hangs on itself. The note's five drawing calls (P3), the beam's lines (P4), the staff and its header (P5) and **every slur and tie arc** (U1) are all in the scene. ⭐ That gap is the migration's remaining work, and `npm run lint:paint` reports the same number from the other side |
