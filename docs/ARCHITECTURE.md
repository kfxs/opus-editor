...
| Ctrl-clicking a MARK into the group | `interactions/markGroupSelect.ts` — ⭐ it RE-RUNS the press chain (filtered to the kinds `selectedItems` can hold — `MarkKind`) with a different TAIL: `pick` records instead of replacing, every drag and the editor door are no-ops. ⛔ Never a second hit-test: which mark a press lands on is `ELEMENT_HIT_ORDER`'s answer, padding, note-body guards, priority and all. `SelectionController.toggleMark` is `toggleNote`'s twin. See `docs/passage-selection-marks.md` |
