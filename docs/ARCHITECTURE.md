...
> softmax (VexFlow's, ported into `rendering/columnFormat`) no longer decides anything horizontal (⏸️ one exception, his call: a clef change after a bar's
...
glyph off a canvas** — VexFlow's way, kept by the port in `rendering/glyphPainter`
...
can `registerFont`, and the injection point would be `rendering/glyphPainter`'s shared measuring
