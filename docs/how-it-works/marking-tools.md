...
| **silence a whole bar** | `barRest` | *nothing — it reads the ACTIVE voice at the click* | replaces that voice of the clicked bar + staff with ONE **stamped** full-bar rest, which keeps its voice alive (a voice 2–4 of rests only otherwise collapses). ⭐ ONE click, then back to selection mode — the only stamp that disarms itself. No ghost: the blue cursor (`interactions/stamps/barRestStamp.ts`, `docs/plans/voice-measure-rest-plan.md`) |
...
see. See docs/plans/time-signature-window-plan.md §4.
...
for one rule; see `docs/history/caret-is-not-a-selection-plan.md` for the general fix (and the live bug it
also repairs).
