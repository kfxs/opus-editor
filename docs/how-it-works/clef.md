...
>
> 📄 The EVIDENCE for every engraving number here is `docs/research/clef-research.md` (the books and the three
> engine clones, question by question) and its predecessor `docs/research/clef-spacing-research.md` (the
...
| | what it is | status |
|---|---|---|
| **owning the clef's INK** | moving the glyph off VexFlow into `engrave/header/clef` — P5b of `docs/plans/own-engraving-engine.md` | ✅ done — VexFlow is removed (2026-09-19); it moved **no pixel** by design |
...
1. ⭐⭐ **The RULE: a clef change goes BEFORE the barline.** Settled — four books unanimous (row 1
   ...
   Only Verovio puts it after. See `docs/research/mid-bar-sign-spacing-research.md`.
...
| | |
|---|---|
| ⭐ **Crossing the barline is a RE-ANCHOR, so it CLEARS the nudge** | the slur endpoint's rule (`docs/plans/slur-endpoint-offset-plan.md`). ⛔ Without it the glyph JUMPS at the switch: the same offset means something different measured from the other side of the line |
...
once by the staff group's own `scale(k)` (`docs/plans/staff-size-plan.md`) and again by VexFlow's ⅔ when the
...
(`docs/research/smufl-fonts-research.md`).
...
ligature — can draw exactly that (`\clef "alto_2"`). ⇒ `docs/plans/octave-clefs-plan.md`'s `(clef, ±octave)`
...
clef's whole nature (`docs/research/clef-research.md` §7.2), and it is the reason a fifth C-clef position would
...
a fifth clef but *a staff whose lines are not pitches* — see `docs/plans/unpitched-staves-plan.md`. Adding
...
⏳ **Octave clefs (`treble8vb`, `bass8vb`) are PLANNED and unbuilt** — `docs/plans/octave-clefs-plan.md`,
...
⭐⭐ **Two things the 2026-09-02 research says about that plan** (`docs/research/clef-research.md` §8.7),
...
- 🚨🚨 **There is no `Score.clef` field and there must never be one.** It existed, it conflated *"the
  ...
  staff's clef changed every staff that had not set its own. `docs/plans/clef-model-plan.md` is the whole
...
2. ⛔ **The m1 b0 opening clef cannot be deleted, on ANY staff** — `removeClefAt`'s guard, decision
   **(b)** of `docs/plans/clef-model-plan.md` §4, HIS choice, *"for symmetry"*. It can only be changed.
...
**0.6** for treble. First noticed 2026-08-29 (`docs/research/clef-spacing-research.md` §A.7). ⚠️
...
The gaps either side of the clef are **not** this file's to re-decide — `docs/research/header-spacing-research.md`
...
(`docs/plans/key-signature-plan.md` §4.2). ⛔ Do not "fix" the divergence — it is two different decisions
...
**P5b of `docs/plans/own-engraving-engine.md`** is under way. Read the seam as it is:
...
⛔ there must not be one. (`docs/plans/staff-size-plan.md`; the standing trap is visual coordinates
...
|---|---|---|---|---|
| **A** | **the clef's INDENTATION** — how far its ink sits inside the staff's left edge | **0.7 sp** | HIS, **2026-09-01** (`docs/research/header-spacing-research.md` §8 A) | `headerInk.CLEF_INDENT` + `rendering/clefIndentPass` |
...
| — | **no `Score.clef`** — clef is per-staff content, resolved positionally | — | `docs/plans/clef-model-plan.md`; the hot fix was **explicitly rejected** by him | `clefOps` + `clefUtils` |
...
| — | **the model stores WRITTEN pitch** under an octave clef | — | `docs/plans/octave-clefs-plan.md` §1 (sounding pitch explicitly rejected) | ⏳ unbuilt |
| — | **no percussion clef** in the `Clef` union | — | Clef window design, 2026-07-20 | `docs/plans/unpitched-staves-plan.md` |
...
⭐ Evidence: **`docs/research/clef-research.md`** (2026-09-02, general) and **`docs/research/clef-spacing-research.md`**
...
the staff group and again by VexFlow's ⅔. `docs/research/clef-research.md` §9.1.
...
| you are… | read |
|---|---|
| choosing an engraving number | `docs/research/clef-research.md` (general) · `docs/research/clef-spacing-research.md` (position & spacing) · `docs/research/header-spacing-research.md` (the run) |
| moving the drawing off VexFlow (✅ done, 2026-09-19) | `docs/plans/own-engraving-engine.md` §P5 · `src/engine/engrave/header/clef.ts` |
| changing the model | `docs/plans/clef-model-plan.md` · `docs/plans/octave-clefs-plan.md` |
| touching the header's other symbol | `docs/plans/key-signature-plan.md` — the closest sibling in every respect |
