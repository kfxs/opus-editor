...
| **S11** | **Ghosts**: each builder (`GhostRenderer.ts:83` note, `:426` rest, `:512–1024` the tool ghosts, `FanGhost.ts:45`) rebuilt on the engrave modules + a ghost style — §8.1's *"a ghost is a scene with a style"* — ⭐ **split (2026-09-18)**, one builder family at a time, each proved by a BROWSER A/B (a ghost is parked by its ink box, which jsdom cannot measure: every drawn element's glyph, screen box, fill, stroke, opacity and face, for every cursor ghost at two pointers and the note ghost at several, HEAD vs now): ✅ **S11a** the clef + meter ghosts — `rendering/HeaderSignGhost`, drawn by the header's own `EngravedClef`/`EngravedTimeSignature` on our surface where a line-less VexFlow `Stave` was built to paint one sign; the sweep-into-a-group and centre-on-pointer wrapper is `ghostCursor.sweepIntoGhostGroup`/`centreGhostOnCursor`. Verified: all 78 ghost cases identical; a small clef instead of the default is caught on all 8 clef cases. Census 1,201 → 1,179 · ⏭️ **S11b** articulation, accidental, dot, tremolo (a dummy note + `Voice` + `Formatter` each, to place one mark) · **S11c** dynamic, tempo · **S11d** rest, fan · **S11e** the note ghost (a whole bar) | **260 / 2** — the largest step | S6, S5, S8, S9 | ⚠️ ghosts become the real engraving; e2e `ghosts` is the net | `GhostRenderer`/`FanGhost` import no `vexflow` |
