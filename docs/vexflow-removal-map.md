...
| **S9h** | `AlignRestsToNotes` ported; our tick contexts; the softmax gone — `spacingPass` writes every context, and the bar-end clef gets a written x | ⚠️ **the bar-end clef moves** unless its softmax x is reproduced — HIS call when we get there |
