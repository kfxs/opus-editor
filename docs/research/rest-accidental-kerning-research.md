...
> we DECIDED. The decision it feeds is `docs/plans/font-metrics-plan.md` §3.6 item 6, still open.
>
...
- `docs/research/spacing-model-research.md:305-316` describes MuseScore's mechanism: `Shape`, pair padding
  skipped when vertically clear, `KerningType`. It quotes **note↔rest 0.5** but says nothing about
  rest↔accidental.
- `docs/research/mid-bar-sign-spacing-research.md:81-83` covers the same mechanism, with padding scaled by
  the mean `mag()`.
- `docs/research/accidental-dot-engines.md:53-67, :486` covers accidental→**notehead** in all four engines
  (LilyPond skylines, MuseScore and Verovio cut-outs, VexFlow bbox). It does not cover a rest.
- `docs/research/multi-voice-rest-position.md` §4.2–4.3 covers the **vertical** placement of voice rests
  (Verovio's accidental-keyed table, MuseScore's `voiceOffset`). It does not cover horizontal
  kerning.
- `docs/plans/font-metrics-plan.md` §3.4 / §3.4a / §3.6 item 6 (`:235-280`, `:358-361`) names this exact
...
whole-staff band. ⚠️ The accidental side is, per `docs/research/accidental-dot-engines.md:486`, a shape with SMuFL
...
(`docs/research/multi-voice-rest-position.md` §4.2).
...
- **A rest's band is real now, and the brief's quotation is stale.** The comment *"A rest's own band is
  not modelled … a band covering the staff"* (quoted at `docs/plans/font-metrics-plan.md:244` as
...
- **Voices:** a moved voice rest exists (`src/engine/layout/restVoicePlacement.ts:222`,
  ...
  `docs/plans/multi-voice-rest-position-plan.md:503-508` records this as a latent disagreement that *"becomes
  one the day a rest row is added"*.
- **The open decision:** `docs/plans/font-metrics-plan.md:358-361` (§3.6 item 6) — *"Should a REST kern against
