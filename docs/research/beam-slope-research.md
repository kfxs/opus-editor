...
> 📄 Written for **P4b** of `docs/plans/beam-engraving-plan.md`. ⛔ **This document builds nothing.** It is
> the research §6.1 of `docs/plans/own-engraving-engine.md` requires *before* code — the same shape as
> `docs/research/stem-length-research.md`, and written after it deliberately, because that one taught the
...
`docs/research/spacing-model-research.md` §§1b–1f. The three findings that bear on this document:
...
⛔ #1 waited for P3e — it is where the ruler, the stem lengths and the ink all meet. ⭐ **P3e landed on 2026-09-16** (`engrave/notes/stemLength`, S6e), so the gate is the sentence below rather than the code: ⚠️ the stem's own REACH is ours, and how much a BEAMED note extends its stem is ours too since S7b (`engrave/beams/beamedStems`, VexFlow's rule transcribed) — and #1's straddle supplement moves exactly that. ⇒ #1 is an edit to that module, and is no longer blocked on the stem's length.
