...
`docs/research/engraving-number-inventory.md` filed these as taste (T) or unsure (?) — this document answers them:
...
- `docs/plans/tremolo-plan.md` §4: why the stack is centred, the FIT and FLAG stretches, and the bbox trap.
- `docs/plans/two-note-tremolo-plan.md` §2: the tip anchor, the beam spending a stroke, and joined/open.
- `docs/plans/tuplet-extension-plan.md` §9: the mark as runs, and the three bracket-end modes.
- `docs/plans/tuplet-control-plan.md` §1–2: VexFlow's `Tuplet` boundary and its hard walls.
- `docs/research/staff-line-research.md` §4.3: the thin-line family's blast radius.
...
2. **Claimed:** `rendering/thinLineWeight.ts:6` and `:23` name `tupletBracketThickness` and *"tuplet brackets"* as members of the one weight. `:84` says *"`THIN_LINE_SPACES` remains the one weight for barlines, ledger lines, octave lines and tuplet brackets"*, and `:60` sets that weight to the font's **0.16 sp**. `docs/research/staff-line-research.md:390-391` repeats the claim.
