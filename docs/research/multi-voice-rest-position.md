...
| ⭐ **our `restShift.steps`** | a **key-line** (VexFlow's unit, kept by our `EngravedNote` since the removal), added in `NoteBuilder` — **= 1 staff SPACE = 2 staff positions** | `types/music.ts`, `docs/plans/rest-shift-plan.md` |
...
spaces = 12 staff positions**, i.e. **double** Gould's displacement. ⚠️ `docs/plans/rest-shift-plan.md`
