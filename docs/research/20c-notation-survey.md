...
| the marking-tool union and `SelectedElement` union | `interactions/EditorState.ts`, `docs/how-it-works/marking-tools.md` | nothing — ⭐ these are the **right** shape for all of §6/§8: a new family is a new member plus a row in `ELEMENT_SPECS` / `ELEMENT_HIT_ORDER`. This survey found no practice that these two unions could not absorb. |
