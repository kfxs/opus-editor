...
>
> **Findings only**, in the manner of `docs/research/spacing-model-research.md`. ⛔ Do not treat any "what a
...
- **Elaine Gould, *Behind Bars: The Definitive Guide to Music Notation*** (Faber Music, 2011, 704
  ...
  standing reference for the *engraving* of all of this (`docs/research/spacing-model-research.md` already
...
- **(a) proportional SPACING** — ordinary note-value symbols, laid out so x-distance ∝ duration.
  ...
  we — it is a number in the spacing rule (`docs/plans/spacing-model-plan.md`).
...
We already draw these (`docs/plans/fanned-beams-plan.md`, `FanPass`), so this is the axis where the survey
...
- ⭐ **The scale is ONE reference duration, and the way you pick it is our `shortestDuration`
  ...
  `docs/plans/shortest-duration-plan.md` and `project_reference_duration` — *the spring is `space(shortest
...
- 🚨 **There is no "Metric modulation" range** ✅ — which matters to us, because
  `docs/plans/metric-modulation-plan.md` already ships the building-block glyphs. The two arrows live in
...
- **Octave clefs are an INTEGER, not a separate sign.** `<clef-octave-change>` — *"used for
  ...
  `@clef.dis` and `@clef.dis.place` ✅.) ⭐ Which matches `docs/plans/octave-clefs-plan.md`'s framing
...
⭐ We already know this is a model gap and have written it down: `docs/plans/unpitched-staves-plan.md` says
...
independently (`docs/plans/multisystem-slur-plan.md`) — worth recording as a rare case where the survey
...
| staff line count is never modelled — `numLines` appears only as VexFlow's `{ numLines: 0 }` on throwaway ghost staves (⚠️ 2026-09-19: VexFlow is removed; the count is now a render-side value only — `EngravedStave`'s `numLines` (5), `StaffFrame.lineCount`, a ghost's `lineCount: 0` — and still absent from the model) | `src/engine/rendering/ghosts/GhostRenderer.ts`, `FanGhost.ts` (today `EngravedStave.ts`, `HeaderSignGhost.ts`) | ✅ **single-line and n-line staves (§7).** Already written up: `docs/plans/unpitched-staves-plan.md` says the percussion clef was left out of the Clef window *because it is a model gap, not a missing row*. |
| the marking-tool union and `SelectedElement` union | `interactions/state/EditorState.ts`, `docs/how-it-works/marking-tools.md` | nothing — ⭐ these are the **right** shape for all of §6/§8: a new family is a new member plus a row in `ELEMENT_SPECS` / `ELEMENT_HIT_ORDER`. This survey found no practice that these two unions could not absorb. |
...
`docs/plans/spacing-model-plan.md` §3c.
