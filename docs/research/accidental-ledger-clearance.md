...
1. ⛔ **The room may not be RESERVED.** Padding the accidental's width and letting the formatter buy
   ...
   key (`MeasureLayout.clefWidthIndependence.test.ts`, `docs/history/render-performance-plan.md` §9); that
...
`docs/research/dot-placement.md`, which reuses this invariant (its rule is uniform per dot, so it may buy its
