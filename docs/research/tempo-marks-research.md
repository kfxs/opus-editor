...
2. **`Score.tempo` has exactly the smell we just deleted from `Score.clef`.** It is a *global
   ...
   made the clef bleed across staves (`docs/plans/clef-model-plan.md`). The fix there was: delete the
...
> by our own `rendering/TempoLayout` (`drawTempoText`), and the ghost by `rendering/TempoGhost`. The
...
- Reuse the existing `DomTextEdit` / `TextEditController` double-click overlay
  (`docs/plans/text-editing-plan.md`). The edit must preserve the mark's id and leave `bpm` untouched.
