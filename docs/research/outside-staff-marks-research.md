...
It is batch 1 of `docs/research/engraving-number-inventory.md` §6.
...
decided and sourced (`docs/how-it-works/above-staff-ladder.md`, `docs/plans/ottava-plan.md` §1 rule 5 + P9).
...
`docs/plans/trill-plan.md` §1 rule 8 + §10 P2, `docs/research/tempo-marks-research.md` §3, and `reference/README.md`.
...
- Italic TEXT parens for the ottava are **his call** (`docs/plans/ottava-plan.md` §"HIS EYE, 2026-08-13"
  item 3).
...
- Still **owed to his eye**:
  - `TRILL_LINE` (`docs/plans/trill-plan.md` §10 P2);
  - `OTTAVA_LINE`, the ottava paren scale/raise and the glyph size (ottava-plan "still open");
  - `PEDAL_LINE` and the pedal glyph size (`docs/plans/pedal-plan.md` §12.1–3);
...
1. **`TRILL_LINE.padding`: the source conflict** (inventory §5b).
   ...
   - **The values ARE LilyPond's**: `padding` 0.5 at `define-grobs.scm:4087`, `staff-padding` 1.0 at
     `:4089`. `docs/plans/trill-plan.md` §1 rule 8 took them as starting numbers. So the ottava comment is
...
3. **"px" in the glyph-size comments.**
   ...
   - `pedalStyle.ts:56-58` and `docs/plans/pedal-plan.md` §12 item 3 say *"at `STAFF_SPACE_PX` = 10 this is
...
- **Gould's paren size or raise** for an octave continuation. Her pp. 28–34 contents list *optional
  parentheses* (`docs/plans/ottava-plan.md` §1), but no parenthesised plate was measured.
