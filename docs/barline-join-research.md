...
> session.** The PLAN is `docs/barline-join-plan.md`; this is the evidence under it. Revisions
...
- **No treatise states what happens to REPEAT DOTS on a joined barline.** Already established and
  recorded in `docs/barline-types-plan.md` §4.5; re-confirmed here. All three engines agree the LINES
...
| a **per-staff span** (MuseScore's `Staff::barLineSpan` / `spanFrom` / `spanTo`) | **nothing we have.** Closest is `docs/barline-types-plan.md` §2's stored-but-unread per-staff scope — but that scope answers *"which staves carry this SIGN"*, which is a different question from *"how far down does this LINE reach"*. ⚠️ Do not conflate them. |
...
24. **What happens to REPEAT DOTS on a joined barline** — no treatise says. Already recorded in
    `docs/barline-types-plan.md` §4.5 and re-confirmed here; the fact rests on three engines agreeing.
...
| MuseScore | ours | note |
|---|---|---|
| `BarLine` (an item per staff per segment, id'd, selectable, with its own properties) | **nothing** — `docs/barline-types-plan.md:79-81`: *"It is not an object. There is no `Barline` type, no id, no field: `score.measures` IS the barline spine"* | ours is a boundary between `Measure[n]` and `Measure[n+1]` |
...
| Barline **grip** + `endDragGrip` snap-to-nearest-staff | nothing — our barline selection is `{ kind: 'barline', measure: N }`, staff-less by design (`docs/barline-types-plan.md:82-84`) | |
...
⚠️ **A correction to our own notes.** `docs/barline-types-plan.md` §2 says *"in all three, barline
...
`BarlineStatement` ~2021, `RepeatStart` ~2041), `docs/barline-types-plan.md` §2 + §4.5,
`docs/multi-staff-plan.md` §1 + §11.
...
- The MuseScore leg of the "dots do not span" claim was **not** re-verified from source in this
  pass; it rests on the earlier survey recorded in `docs/barline-types-plan.md` §4.5.
- The treatises: not consulted here, and `docs/barline-types-plan.md` §4.5 already records them as
