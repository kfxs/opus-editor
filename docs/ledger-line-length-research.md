...
> `docs/staff-line-research.md`, which settled the ledger's **thickness** — ⛔ not reopened here.
>
...
Stone's *Leger Lines* is printed **p. 30**, not 31 (`docs/staff-line-research.md` §1, §2.3). G&L's
...
(`docs/accidental-dot-research.md` row A5, searched). Her only licence to shorten is *"cramped
conditions"*.
...
because the 1970 photo-offset has ink gain (see `docs/staff-line-research.md` §3.4). The overhang
...
- **Accidental**: a ledger inside the accidental glyph's per-glyph vertical band gets its **left end
  ...
  Only the left end moves. Band values are in `docs/accidental-dot-engines.md` A5.
...
- **Accidental**: ⛔ no shortening. The code is written and commented out (`chordlayout.cpp:1361-1365`);
  the sign may overlap the ledger instead (`docs/accidental-dot-engines.md` A5).
...
| Gonville | ⛔ ships no metadata (`docs/smufl-fonts-research.md` §3) | | |
...
- **Spacing reserve**: `INK.ledgerLeft` **0.30**, `INK.ledgerRight` **1.50** from the head anchor, 1.80 sp
  ...
  (`docs/font-metrics-plan.md:356-357`).
  🚨 `docs/note-engraving-plan.md:688` and §3.1 (`:706-710`) say the ink table *"already reserve[s]
...
After his report of accidentals colliding with ledger lines (`docs/accidental-ledger-clearance.md`), the
...
Recorded as A5 in `docs/accidental-dot-research.md:244`. ⏭️ **The asymmetric version** (LilyPond: left
end only, to the midpoint) is listed as a possible future row, *not taken*
(`docs/note-engraving-plan.md` §4, `:855-857`).
