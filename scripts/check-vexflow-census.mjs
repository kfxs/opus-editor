...
function role(u) {
  ...
  // `preFormatted` / `postFormatted` (S12j-d2): the formatter's own flags, read by the note's transcribed
  // `preFormat` / `setStemDirection` / `postFormat` — inside the library until now. They go with the class (S12j-e).
  if (c === 'Tickable' && /^(preFormatted|postFormatted)$/.test(m)) return 'R4 formatter'
