...
export function createEditorApp(host: HTMLElement): EditorApp {
  ...
  function installPerfInstruments(): void {
    ...
    dbg('[spine] a bent staff, LIVE from the open score: __spine.circle({ notes: 8 }) loads fourths · .show({ radius, size, zoom }) bends what is open — size = the MUSIC\'s (1 = the page\'s staff) on the same circle, zoom = the CANVAS\'s · .straight({ size, zoom }) / .clear()')
