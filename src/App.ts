...
export function createEditorApp(host: HTMLElement): EditorApp {
  ...
  function syncFromState(): void {
    // The score box's cursor: hidden while panning, the blue placement pointer under an armed paste
    // or a tool that arms with no ghost (expression / tempo / slur stamp).
  ...
  function installPerfInstruments(): void {
    ...
    // ⚠️ EXPERIMENT, HIS (2026-09-02) — how far the header→first-note gap closes in front of an
    ...
    dbg("[dots] augmentation-dot gaps: __dots.gap('house'|'gould'|'gouldDrawn'|'ross'|'lilypond'|'musescore'|'verovio'|'vexflow') / .dump() / .reset() — ⚠️ look at a DOUBLE dot · a REST's: __dots.restGap('gould'|'followNotes'|'lilypond'|'musescore'|'verovio'|'vexflow') / .restReset()")
