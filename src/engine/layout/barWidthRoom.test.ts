...
/** The surface every case below is cast off onto — today's sketching canvas. */
const SURFACE = resolveSurface(SKETCH_CANVAS)

/** A full line: the available width, exactly — so the "does this line fill the page?" test passes. */
const AVAILABLE = SURFACE.contentWidthPx
...
describe('barWidthRoom — what it declines to answer', () => {
  it('a bar nobody has drawn has no room', () => {
    expect(barWidthRoom({
      measureNumber: 7, layout: lineOf([bar(1, 200)]), stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50,
    })).toBeNull()
  })

  it('a bar with no note space cannot be multiplied', () => {
    const layout = lineOf([bar(1, 200, { noteSpace: 0 })])
    expect(barWidthRoom({ measureNumber: 1, layout, stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })).toBeNull()
  })
})
...
describe('barWidthRoom — linear view justifies nothing', () => {
  it('a px of stretch space is a px of barline movement, both slopes 1', () => {
    ...
    const room = barWidthRoom({ measureNumber: 1, layout, stretch: 1, viewMode: 'linear', surface: SURFACE, slackPx: 50 })!
...
describe('barWidthRoom — on a justified line, growth is a TRANSFER', () => {
  ...
  it('the grown bar takes its growth whole — widthSlope 1', () => {
    const room = barWidthRoom({ measureNumber: 1, layout: layout(), stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
    expect(room.widthSlope).toBe(1)
  ...
  it('the barline after it moves by what the payers SITTING BEFORE IT give up', () => {
    ...
    const first = barWidthRoom({ measureNumber: 1, layout: layout(), stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
    ...
    const last = barWidthRoom({ measureNumber: 2, layout: layout(), stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
...
describe('barWidthRoom — the floors and the ceiling', () => {
  it('the shrink floor is the room the DRAWN music is not using', () => {
    ...
    const room = barWidthRoom({ measureNumber: 1, layout, stretch: 2, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
    expect(room.minStretch).toBeCloseTo(1.5, 6)
  })

  it('and never goes under the absolute minimum, however much slack is measured', () => {
    const layout = lineOf([bar(1, AVAILABLE / 2), bar(2, AVAILABLE / 2)])
    const room = barWidthRoom({ measureNumber: 1, layout, stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 10_000 })!
    expect(room.minStretch).toBe(BAR_STRETCH_MIN)
  })

  it('the ceiling is the stretch at which the bar becomes the WHOLE LINE', () => {
    const layout = lineOf([bar(1, AVAILABLE / 2), bar(2, AVAILABLE / 2)])
    const room = barWidthRoom({ measureNumber: 1, layout, stretch: 1, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
  ...
  it('⭐ a bar ALONE on a full system is already as wide as anything can make it', () => {
    ...
    const room = barWidthRoom({ measureNumber: 1, layout, stretch: 4, viewMode: 'wrapped', surface: SURFACE, slackPx: 50 })!
