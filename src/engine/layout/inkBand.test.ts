...
import {
  clearanceBaseline, columnsBetween, columnsUnder, mergeInkBands, staffInkBand,
  type Clearance, type InkBand, type MarkInk, type StaffSide,
} from './inkBand'
 ...
 * PARAMETERISED, so a second family gets its own line from its own constants rather than from a
 ...
 * (`reference_jsdom_cannot_measure_glyphs`). Whether the drawn marks land on the drawn line is the
 * browser suite's.
 */
 ...
 *  rule being parameterised, so it must not depend on what any one family currently sets — importing
 *  `DYNAMICS_LINE` here would make a tuning change look like a rule change. */
const DYN: Clearance = { padding: 0.6, minFromStaff: 2.1 }

/** The moved assertions were all written against the dynamics' numbers; keep them so, once. */
const BASE = (band: InkBand | null, side: StaffSide) => clearanceBaseline(band, side, MARK, DYN)
...
describe('⭐⭐ clearanceBaseline is PARAMETERISED — the point of the extraction', () => {
  ...
  it('two families with two constant sets get two different lines from the same band', () => {
    const staffResident = { top: 0, bottom: 4 }
    expect(BASE(staffResident, 'below')).toBeCloseTo(4 + DYN.minFromStaff + MARK.above, 10)
    expect(clearanceBaseline(staffResident, 'above', MARK, TRILL))
      .toBeCloseTo(-TRILL.minFromStaff - MARK.below, 10)
  })
  ...
  it('⭐ the trill sits NEARER the staff than the dynamics would — it is the innermost family', () => {
    ...
    const dynamics = clearanceBaseline(band, 'above', MARK, DYN)
    expect(trill).toBeGreaterThan(dynamics) // less negative = closer to the staff
  })

  it('the padding decides once the ink is past the floor', () => {
    const deep = { top: 2, bottom: 9 }
    const near = clearanceBaseline(deep, 'below', MARK, { padding: 0.5, minFromStaff: 1 })
    const far = clearanceBaseline(deep, 'below', MARK, { padding: 1.5, minFromStaff: 1 })
    expect(far - near).toBeCloseTo(1, 10)
  })
})
...
describe('the rule — clear the ink, floored at the staff', () => {
  it('a BELOW line clears the lowest ink by the padding, and the baseline hangs the mark from it', () => {
    ...
    expect(BASE({ top: 2, bottom: 7 }, 'below')).toBeCloseTo(7 + DYN.padding + MARK.above, 10)
  })

  it('⭐ but never closer to the staff than the minimum — a bar of middle-line notes does not pull marks up', () => {
    // The §2.2 defect being fixed: VexFlow hangs the mark off the NOTE, so a high passage puts its
    // `p` inside the staff. The floor is measured from the bottom LINE, not from the music.
    const floor = 4 + DYN.minFromStaff + MARK.above
    expect(BASE({ top: -1, bottom: 1 }, 'below')).toBeCloseTo(floor, 10)
    // …and ink that stops just inside the staff cannot beat it either.
    expect(BASE({ top: 0, bottom: 4 }, 'below')).toBeCloseTo(floor, 10)
  })

  it('a staff with NO ink at all still has a line — the floor is the whole answer', () => {
    expect(BASE(null, 'below')).toBeCloseTo(4 + DYN.minFromStaff + MARK.above, 10)
  })
  ...
  it('ABOVE is the mirror, and it hangs the mark by its OTHER edge', () => {
    ...
    expect(BASE({ top: -3, bottom: 2 }, 'above')).toBeCloseTo(-3 - DYN.padding - MARK.below, 10)
    expect(BASE({ top: 1, bottom: 3 }, 'above')).toBeCloseTo(-DYN.minFromStaff - MARK.below, 10)
  })
})
...
describe('reading the ink', () => {
  it('⭐⭐ a mark clears the ink UNDER IT, and a low note ELSEWHERE leaves it alone', () => {
    ...
    expect(BASE(underTheMark, 'below')).toBeLessThan(BASE(bothBars, 'below'))
  })
  ...
  it('a bar of high notes sits at the floor — outside the staff, where every mark shares it', () => {
    ...
    expect(BASE(band, 'below')).toBeCloseTo(4 + DYN.minFromStaff + MARK.above, 10)
  })
...
describe('which ink is MINE — the staff', () => {
  ...
  it('⚠️⚠️ the FIRST staff\'s boxes carry NO staff id, and asking by its real id must still find them', () => {
    ...
    // marks at the floor while the music hangs three spaces lower.
  ...
  it('⚠️ a low note on the LOWER staff does not move the UPPER staff\'s line', () => {
    ...
    expect(BASE(staffInkBand(columns, lower, upper), 'below'))
      .toBeGreaterThan(BASE(staffInkBand(columns, upper, upper), 'below'))
  })
...
describe('columnsUnder — one column, the mark\'s own', () => {
  const bar = (): Column[] => [0, 1, 2, 3].map(b => ({ ...plainColumn(frac(b, 1), frac(1, 1)), ink: [] }))

  it('takes the column at the beat, else the next one after it (fall-forward)', () => {
    expect(columnsUnder(bar(), frac(2, 1)).map(c => c.beat.num)).toEqual([2])
    expect(columnsUnder(bar(), frac(3, 2)).map(c => c.beat.num)).toEqual([2])
  })

  it('is empty past the last column', () => {
    expect(columnsUnder(bar(), frac(9, 1))).toHaveLength(0)
  })
})
...
describe('columnsBetween + mergeInkBands — the SPANNER\'s wider slice', () => {
  ...
  it('takes [from, to) — a spanner reaches to its end without standing over what begins there', () => {
  ...
  it('⭐ a spanner is measured against every column it covers, not just its first', () => {
    ...
    expect(BASE(band, 'below')).toBeCloseTo(11 + DYN.padding + MARK.above, 5)
  })
