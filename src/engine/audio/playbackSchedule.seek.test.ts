...
describe('playableFrom', () => {
  ...
  it('⭐ from bar 3 (beat 8 of 4/4), drops what came before and re-zeroes the rest', () => {
    ...
    expect(out.map(n => n.midi), 'the first eight beats are not this play').toEqual([68, 69, 70, 71])
  ...
  it('⚠️ drops a note still SOUNDING across the start rather than re-striking it', () => {
    ...
    const held: ScheduledNote = { midi: 50, startBeats: 4, durationBeats: 8, velocity: 0.8 }
    const out = playableFrom([held, ...quarters(12)], STEADY, 8)
    expect(out.some(n => n.midi === 50)).toBe(false)
  })
  ...
  it('keeps a duration as the difference of two lookups, so it survives a tempo change', () => {
    ...
    const straddling: ScheduledNote = { midi: 60, startBeats: 9, durationBeats: 2, velocity: 1 }
  ...
  it('carries pitch and velocity through untouched', () => {
    const out = playableFrom([{ midi: 72, startBeats: 4, durationBeats: 2, velocity: 0.42 }], STEADY, 4)
    expect(out).toEqual([{ midi: 72, atSeconds: 0, durationSeconds: 1, velocity: 0.42 }])
  })
