...
export function buildDynamicAnnotation(dyn: Dynamic): EngravedAnnotation {
  ...
  annotation.setFont(dynamicAnnotationFont())
...
export function enlargeDynamicGlyphRuns(text: SVGTextElement, dyn: Dynamic): void {
  ...
  for (const run of runs) {
    ...
    if (run.glyph) {
      tspan.setAttribute('font-family', musicOnlyStack())
...
export function applyDynamicOffsets(pass: RenderPass, measure: Measure, stave: EngravedStave): void {
  ...
  for (const dyn of measure.dynamics) {
    ...
    setDynamicMarkNudge(
      pass, dyn.id, el, staffSpacesToPixels(off.x, staveFrame(stave)), staffSpacesToPixels(off.y, staveFrame(stave)))
  }
}
