...
describe('note spacing — width (§2)', () => {
  it('the bar grows by exactly the authored space', () => {
    ...
    space(model, 2, 3)
    const after = calculateMeasureWidths(model.getScore(), clefs(model.getScore()), 'linear')
  ...
  it('a bar nobody touched carries no authored space', () => {
    const model = scoreWith(2)
    space(model, 1, 2)
    expect(calculateMeasureWidths(model.getScore(), clefs(model.getScore()), 'linear').get(2)!.userSpace)
      .toBe(0)
  })
...
describe('note spacing — justification (§3)', () => {
  ...
  it('one bar alone on its line cannot hand its space back through the right margin', () => {
    ...
    const info = calculateMeasureWidths(model.getScore(), clefs(model.getScore()), 'wrapped', undefined, true).get(1)!
