...
/**
 * The `curveShape` kind (client #1). The forward-migration that used to be tested here is gone:
 * there is no legacy JSON in the wild, so there is no legacy shape to migrate FROM — see the
 * "no JSON migration" rule. What survives is the read itself.
 */
describe('curveShape override (client #1)', () => {
  const scoreWithSlur = (): Score => ({
    id: 's', title: 't',
    measures: [],
    slurs: [{ id: 'slur-1', startNoteId: 'n-a', endNoteId: 'n-b' } as Slur],
  })

  it('reads back the stored staff-space control points', () => {
    const score = scoreWithSlur()
    const stored: CurveShapeOverride = { kind: 'curveShape', cps: [{ x: 0.5, y: 1 }, { x: -0.5, y: 3 }] }
    score.engravingOverrides = { 'slur-1': [stored] }
    expect(curveShapeOverrideOf(score, 'slur-1')?.cps).toEqual([{ x: 0.5, y: 1 }, { x: -0.5, y: 3 }])
  })

  it('returns undefined when the element has no curve shape', () => {
    expect(curveShapeOverrideOf(scoreWithSlur(), 'slur-1')).toBeUndefined()
  })
})
