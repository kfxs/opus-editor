...
import type { KeySignature, Score, Note, NoteParams, Fraction, Tuplet, TupletFormat, TupletMarkRun, TupletShape, TupletNumberStyle, NoteDuration, ArticulationType, Accidental, PitchSpelling, Clef, TimeSignature, Dynamic, DynamicLevel, Hairpin, Ottava, Pedal, TempoMark, Slur, Trill, PitchAlter, PitchStep, TremoloMark, FanMark, SoundRef, BarlineStyle, StaffGroup, FractionalBeamSide } from '@/types/music'
...
export class MusicEngine {
  ...
  //
  // ⭐ The family's COMMANDS are `engine/commands/pedalCommands` — `engine.pedal.<command>(…)`, the
  // ottava's arrangement above. What stays here is its reads.

  /** Every edit the editor can make to a sustain pedal. */
  readonly pedal = pedalCommands(this.commandContext())
  ...
  // ==================== Hairpin operations ====================
  //
  // ⭐ The family's COMMANDS are `engine/commands/hairpinCommands` — `engine.hairpin.<command>(…)`,
  // the ottava's arrangement. What stays here is its reads.

  /** Every edit the editor can make to a hairpin. */
  readonly hairpin = hairpinCommands(this.commandContext())
  ...
  addNoteAtPosition(
    ...
    /** CUE armed for entry (cue-size-plan). */
    cue?: NoteParams['cue'],
  ): Note | null {
    return this.noteEntryCoordinator.addNoteAtPosition(coords, duration, accidental, dots, articulations, beam, voice, tremolo, enclosure, cue)
  }
  ...
  // ==================== Trills ====================
  //
  // ⭐ The family's COMMANDS are `engine/commands/trillCommands` — `engine.trill.<command>(…)`, the
  // ottava's arrangement. What stays here is its reads.

  /** Every edit the editor can make to a trill. */
  readonly trill = trillCommands(this.commandContext())
  ...
  renderScoreWithPreview(
    ...
    enclosure?: NoteParams['enclosure'],
    /** CUE armed for entry — the ghost is drawn at the cue size, as the note will land (cue-size-plan). */
    cue?: NoteParams['cue'],
  ): boolean {
  ...
  loadJSON(json: string): void {
    ...
    for (const problem of cueProblems(loaded.getScore())) console.warn(`[score-file] ${problem}`)
