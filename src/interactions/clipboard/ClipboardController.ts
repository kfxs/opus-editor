...
export class ClipboardController {
  ...
  private placeAt(target: ClipTarget): void {
    ...
    // ⭐ EVERYTHING the paste wrote is the selection — every note and rest of the window, on every lane
    // the clip landed in (his report, 2026-09-26). `pastedIds` is only the chords it created: a rest the
    // clip carried, or a second voice, was left unselected — highlighted in part.
    const selected = windowSlotIds(engine.getScore(), target, this.payload)
    dbg(`[Clipboard] pasted ${pastedIds.length} note(s) — ${selected.length} slot(s) selected`
