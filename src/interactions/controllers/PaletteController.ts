...
import { promoteStampToNoteEntry } from '../state/stampPromotion'
...
export class PaletteController {
   ...
   *  Public for the tools whose press lives in `stamps/` (the grace tool, via the dev toolbar). */
  spanToolHost(): SpanToolHost {
    return {
      state: this.state,
   ...
   * (`interactions/state/measurePassage`), so "one staff" is simply a span of one.
