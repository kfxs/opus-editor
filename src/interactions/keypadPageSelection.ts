import { DEFAULT_KEYPAD_PAGE, nextKeypadPageId, type KeypadPageId } from '../windows/keypad/keypadLayouts'

/**
 * Which Keypad page is showing — the seam between the panel and the numpad.
 *
 * It lives out here, next to the other Keypad seams (duration/accidental/mode/voice…), and NOT inside
 * {@link KeypadWidget}, for one reason: the numpad IS the Keypad, and it keeps working with the panel
 * closed. A page held privately by the widget would be a page the keyboard cannot see (so `Numpad4`
 * meant "quarter note" forever, whatever the panel was showing) and a page that is forgotten every
 * time the window closes. Here there is always a current page, panel or no panel.
 *
 ...
 *
 * Simpler than a {@link PaletteSelection}: a page has no highlight/press split. It is one current
