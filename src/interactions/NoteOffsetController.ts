 ...
 * docs/plans/note-offset-plan.md §B). The window is a **dumb publisher**: it writes an absolute `{noteId,
...
export class NoteOffsetController {
  ...
  private apply({ noteId, x }: NoteOffsetRequest): void {
    ...
    // current one (docs/plans/note-offset-plan.md §B). No change → nothing to do (and no empty undo entry).
