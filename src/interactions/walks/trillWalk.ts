...
import {
  trillInkX, trillInkY, trillLane, trillLaneIndexAt, trillLaneOnStaff, trillRibbonLimits, trillRibbonX,
  trillSquareBaseX, trillSquareMeasure, trillStaffBand, trillStaffSpacePx, trillSystemNoteFor,
} from '../lanes/trillLane'
...
import { traceTrillFrame } from './trillTrace'
...
export function dragTrillBody(
  ...
  if (debugEnabled()) {
    traceTrillFrame(engine, id, { cursorX, dxPx, dyPx, staffSpacePx },
      { port: bodyPort(engine, id, bodyMoveWrites(engine, id)), staff: trillStaff(engine, id) })
  }
...
function jumpTrillStaves(
  ...
  const inkBefore = trillInkX(engine.getElementRegistry(), id)
