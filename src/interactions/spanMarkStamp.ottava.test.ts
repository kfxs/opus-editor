...
describe("stampSpanMarkAtClick — the ottava's row", () => {
  ...
  it('stands down entirely when the tool is not armed', () => {
    ...
    expect(stampSpanMarkAtClick('ottava', state, engine, hits(ids[1]), 10, 10, render)).toBe(false)
    expect(ottavas()).toHaveLength(0)
  })

  it('puts a line over the note clicked, and repaints', () => {
    expect(stampSpanMarkAtClick('ottava', state, engine, hits(ids[1]), 10, 10, render)).toBe(true)
  ...
  it('⭐ covers EXACTLY the note clicked — asserted in SOUND, which is what the line is for', () => {
    ...
    stampSpanMarkAtClick('ottava', state, engine, hits(ids[1]), 10, 10, render)
  ...
  it('an 8vb stamp puts the line the other way', () => {
    state.selectedMarkingTool = { kind: 'ottava', shift: -1 }
    stampSpanMarkAtClick('ottava', state, engine, hits(ids[0]), 10, 10, render)
    expect(ottavas()[0].shift).toBe(-1)
  ...
  it('CONSUMES a click that missed every note, and changes nothing', () => {
    ...
    expect(stampSpanMarkAtClick('ottava', state, engine, hits(null), 10, 10, render)).toBe(true)
    expect(ottavas()).toHaveLength(0)
  ...
  it('⛔ refuses a REST — there is no sounding music to displace', () => {
    ...
    expect(stampSpanMarkAtClick('ottava', state, engine, hits(rest!.id), 10, 10, render)).toBe(true)
    expect(ottavas()).toHaveLength(0)
  })
  ...
  it('⭐ the LAST note of the score still takes one — a line needs no second anchor', () => {
    ...
    expect(stampSpanMarkAtClick('ottava', state, engine, hits(last), 10, 10, render)).toBe(true)
    expect(ottavas()).toHaveLength(1)
