...
/**
 * Applies a Properties-panel **tempo mark offset** request to the engine (his ask, 2026-08-19).
 *
 * {@link DynamicOffsetController}'s twin, in its shape and for its reasons: the window writes an
 * absolute `{tempoId, x, y}` to {@link bus.tempoOffset}, and this — the one place that holds
 ...
 */
export class TempoOffsetController {
  ...
  constructor(
    ...
    this.unsubscribe = bus.tempoOffset.onSet((req) => this.apply(req))
  }

  private apply({ tempoId, x, y }: TempoOffsetRequest): void {
    ...
    const current = tempoOffsetOverrideOf(engine.getScore(), tempoId)
