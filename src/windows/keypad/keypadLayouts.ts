 ...
 * `npm run bake:keypad`). 🚨 The text form asked the BROWSER to lay each glyph out, and a browser ZOOM
 * re-rounds that layout — the strokes slid against their note at some zoom levels (his report,
 * 2026-09-20). An outline is coordinates; it scales like any path.
 *
 * **To tune a drawing:** change its numbers here and reload — the drawing no longer matches what was
 * baked, so the keypad falls back to the LIVE text form and shows the new numbers at once (or swap
 * `tremolo(` for {@link rework} to see the raw stacked spans). When it looks right, run
 * **`npm run bake:keypad`**: it measures the drawing in a real browser at 100% zoom, writes the
 * outlines, and refuses to write them unless they sit on the text drawing. `keypadBakedIcons.test.ts`
 * goes red while a recipe is tuned and not yet baked.
 */
...
const TREMOLO = {
  ...
  // ⭐ WHERE A BEAM ENDS, measured and not eyeballed (Bravura.otf): a quarter @22 placed at `dx` has its
  // stem's OUTER edge at `13 + dx + 3.65`, and a bar @30 placed at `dx` runs `13 + dx − 5.13` …
  // `13 + dx + 5.13`. The last bar is set to end ~½ unit PAST that stem edge, on purpose: the bar is
  // pixel-snapped and the stem is a grey hairline, so a bar ending exactly ON the edge rounds short
  // at some zooms and the stem is left standing outside the beam (his report, 2026-09-20).
  featheredAccel: tremolo([g(NOTE.quarter, 22, 9, -8), g(NOTE.quarter, 22, 9, 3), g(NOTE.quarter, 22, 9, 16), g('\uE1F8', 30, 13, 0), g('\uE1F8', 30, 13, 8), g('\uE1F8', 30, 13, 15), g('\uE988', 14, -8, 1)]),
  featheredRit: tremolo([g(NOTE.quarter, 22, 9, -13), g(NOTE.quarter, 22, 9, -1), g(NOTE.quarter, 22, 9, 10), g('\uE1F8', 30, 13, -5), g('\uE1F8', 30, 13, 3), g('\uE1F8', 30, 13, 9), g('\uE978', 14, -3, -3)]),
  ...
  twoNoteQuarters: tremolo([g(NOTE.quarter, 22, 12, -12), g(NOTE.quarter, 22, 12, 10), g('\uE4E7', 22, 6, 1), g('\uE1FA', 30, 13, -4), g('\uE1FA', 30, 13, 2), g('\uE1FA', 30, 13, 9), g('\uE204', 22, 5, -1), g('\uE204', 22, 5, 10), g('\uE204', 22, 5, -12)]),
  oneBeam: barred(-2),
  threeBeams: tremolo([g(NOTE_DOWN.quarter, undefined, -10, 4), g('\uE1FA', 30, 31, -5), g('\uE1FA', 30, 31, 4), g('\uE1FA', 30, 31, 7)]),
  oneBeamOffset: barred(-2, 10),
}

/**
 * ⭐ **The recipes that get BAKED to outlines**, by name — what `npm run bake:keypad` reads and
 * `./keypadBakedIcons` (GENERATED) answers for. The recipe above is the SOURCE; the baked outlines are
...
const pageBeamsTremolos: CellSpec[] = [
  ...
  // The FEATHERED BEAMS, now that they are modelled — Sibelius's `0` and `.`, and the last two cells
  // on this page to be wired. A note carries ONE fan, so the pair is a radio like the tremolo counts:
  // the lit one takes it off, the other turns it round. `pressFan` owns both rules already.
  ['feathered accel', TREMOLO.featheredAccel, 'fan', 'accel'],
  ['feathered rit', TREMOLO.featheredRit, 'fan', 'rit'],
]
